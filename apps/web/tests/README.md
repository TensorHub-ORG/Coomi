...
the deliberate composition divergences from `coomi web` — are documented in
...
## Do not import `@coomi/coomi-client-*` here
...
`@coomi/coomi-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@coomi/coomi-client-web` and the boot-manifest
type from `@coomi/coomi-client-modules/client`: booting the real shell is what
...
`@coomi/coomi-client-runtime/client` because `client/runtime` is reachable
