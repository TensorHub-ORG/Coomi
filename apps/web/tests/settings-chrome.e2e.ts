...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('uses the persisted dark preference while plugins are still loading', async () => {
    ...
    try {
      ...
      const state = await loading.evaluate((element) => {
        ...
        return {
          attr: document.body.hasAttribute('data-coomi-dark-theme'),
    ...
    await expect.poll(() => page.evaluate(() => document.body.hasAttribute('data-coomi-dark-theme')), {
      timeout: 5_000,
    }).toBe(false)
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    const readState = async (target: Page = page): Promise<ThemeState> => await target.evaluate(() => {
      ...
      return {
        ...
        legacy: localStorage.getItem('coomi.theme'),
        themeColor: metas[0]?.content ?? null,
        themeColorCount: metas.length,
        token: computed.getPropertyValue('--coomi-alias-bg-base').trim(),
      }
    })
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('coomi.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
    ...
    try {
      ...
      expect(await secondPage.evaluate(() => localStorage.getItem('coomi.conversation.busyEnter'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('coomi.locale'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/locale:\n\s+preference: en/)
    ...
    try {
      ...
      expect(await secondPage.evaluate(() => localStorage.getItem('coomi.locale'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    try {
      ...
      expect(await enPage.evaluate(() => localStorage.getItem('coomi.locale'))).toBeNull()
