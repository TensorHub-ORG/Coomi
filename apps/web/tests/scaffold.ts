...
// Boots the REAL web composition — the coomi-base and coomi-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $COOMI_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts coomi-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `coomi web`, all deliberate, all via include
...
import Include, { type PatchOptions } from '@coomi/cordis-plugin-include'
import Group from '@coomi/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@coomi/coomi-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@coomi/coomi-app-boot'
import { coomiHomePath } from '@coomi/coomi-home-paths'
import { settingsNamespace } from '@coomi/coomi-settings'
import { LlmAdapter } from '@coomi/coomi-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@coomi/coomi-llm'
import type { ReplayHandle } from '@coomi/coomi-llm-replay'
import { installLlmReplay, parseSessionLog } from '@coomi/coomi-llm-replay'
import SessionStore, {
  packChunkRuns,
  ...
  type SessionEvent,
  type SessionHeader,
} from '@coomi/coomi-session'
import JsonlSessionPersistence from '@coomi/coomi-session-persistence-jsonl'
// Empty type imports carry the webServer/agents/sessionPersistence Context merges.
import type {} from '@coomi/coomi-host-webserver'
import type {} from '@coomi/coomi-agent'
import { provideCmdline } from '@coomi/coomi-cmdline'
...
// } from '@coomi/coomi-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'Coomi 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 Coomi 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 Coomi 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $COOMI_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.COOMI_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`COOMI_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the coomi-base and coomi-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($COOMI_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted coomi-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'coomi-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $COOMI_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.coomi document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.coomi-home')
  ...
  // session is created. Without this a developer's real ~/.coomi/skills silently
  // enters replay requests and goldens while CI sees none. `COOMI_HOME` follows
  ...
  const skillRootEnvironment = {
    ...
    COOMI_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    COOMI_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'coomi-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in coomi.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.coomi/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $COOMI_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.coomi-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.coomi, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        coomiHome: join(workspaceCwd, '.coomi-home'),
    ...
    // to the production OTLP endpoint (or whatever COOMI_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @coomi/coomi-web-frontend); only the URL line is silenced.
    ...
    { id: 'settings', config: { coomiHome: harnessHome } },
    { id: 'credentials', config: { coomiHome: harnessHome } },
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@coomi/coomi-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@coomi/coomi-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@coomi/coomi-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    ctx.provide('coomiHomePath', coomiHomePath)
    ...
    // `@coomi/cordis-plugin-group` by name.
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run COOMI_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
