/** Published coomi web + pnpm dev:web → browser HMR, with no page reload. */
...
import type { SubprocessHandle, SubprocessSpawnSpec } from '@coomi/coomi-subprocess'
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  const world = await mkdtemp(join(tmpdir(), 'coomi-web-hmr-world-'))
  ...
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built coomi bin; run pnpm run build first')
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        ...
        COOMI_HOME: join(world, '.coomi'),
      },
    ))
    const baseUrl = await waitForOutput(host, /coomi web: (http:\/\/[^\s]+)/, 'built coomi web')
    ...
    const pageIdentity = await page.evaluate(() => {
      ...
      Object.defineProperty(window, '__coomiHmrPageIdentity', { value: identity })
      return identity
    })

    await writeFile(sourcePath, updatedSource)
    await page.getByText(newText, { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.evaluate(() => (window as Window & { __coomiHmrPageIdentity?: string }).__coomiHmrPageIdentity))
      .toBe(pageIdentity)
