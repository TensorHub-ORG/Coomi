...
interface ListMetrics {
  ...
  hoverRules: string[]
  /** `--coomi-scrollbar-thumb` resolved on the list, serialized as a colour. */
  token: string
  /** `--coomi-scrollbar-thumb-hover` resolved on the list, serialized the same way. */
  hoverToken: string
...
function measureList(page: Page): Promise<ListMetrics> {
  return page.evaluate(() => {
    ...
    return {
      ...
      hoverRules,
      token: resolve('--coomi-scrollbar-thumb'),
      hoverToken: resolve('--coomi-scrollbar-thumb-hover'),
...
interface PaletteMetrics {
  ...
  hovered: ListMetrics
  /** `--coomi-scrollbar-thumb` with the pointer parked outside the column. */
  quietThumb: string
}
...
function renderGeometry(light: PaletteMetrics, dark: PaletteMetrics): string {
  const palette = (name: string, { hovered: metrics, quietThumb }: PaletteMetrics): string[] => [
    `## ${name}`,
    '',
    `- --coomi-scrollbar-thumb, pointer outside the sidebar: ${quietThumb}`,
    ...
    `- --coomi-scrollbar-thumb, pointer over the list: ${metrics.token}`,
    `- --coomi-scrollbar-thumb-hover, pointer over the list: ${metrics.hoverToken}`,
...
/**
 * Resolve `--coomi-scrollbar-thumb` as the list sees it, without the rest of the
...
function resolveThumb(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    if (list === null) throw new Error('sidebar session list not in the DOM')
    const probe = document.createElement('span')
    probe.style.color = 'var(--coomi-scrollbar-thumb)'
    list.append(probe)
    const value = getComputedStyle(probe).color
    probe.remove()
    return value
...
describe('web e2e: sidebar session list scrollbar (reserved gutter / themed thumb)', () => {
  ...
  it('renders the themed thumb through the WebKit path in both palettes', async () => {
    ...
    expect(light.hoverRules).toEqual(['var(--coomi-scrollbar-thumb-hover)'])
    ...
    await page.evaluate(() => { document.body.setAttribute('data-coomi-dark-theme', '') })
    ...
    await page.evaluate(() => { document.body.removeAttribute('data-coomi-dark-theme') })
