// Real-host smoke: spawn `coomi web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`coomi web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /coomi web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`coomi web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `coomi web` serves every production manifest plugin; until every UI
...
describe('coomi web keyless CLI smoke', () => {
  it('listens on 127.0.0.1 by default', async () => {
    requireDist()
    const sessionsDir = mkdtempSync(join(tmpdir(), 'coomi-web-keyless-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          COOMI_HOME: join(sessionsDir, '.coomi'),
          COOMI_AGENTS_HOME: join(sessionsDir, '.agents'),
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'coomi-web-workspace-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          COOMI_HOME: join(workspace, '.coomi'),
          COOMI_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'coomi-web-retry-'))
  ...
  it('COOMI_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'coomi-web-code-mode-'))
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    requireDist()
    sessionsDir = mkdtempSync(join(tmpdir(), 'coomi-web-w5-'))
    const port = await probeFreePort()
    // tsx boot mirrors the runtime half of the root coomi script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $COOMI_HOME with the
    ...
    await page.waitForFunction(
      () => document.title !== 'Coomi' && document.title.endsWith(' — Coomi'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Coomi`,
      durableTitle,
      { timeout: 15_000 },
    )
  ...
  it('dark mode: the body attribute cascades the token sheets', async () => {
    ...
    const dark = await page.evaluate(() => {
      document.body.setAttribute('data-coomi-dark-theme', '')
      return getComputedStyle(document.body).backgroundColor
    })
    await screen(page, '11-dark-mode')
    const light = await page.evaluate(() => {
      document.body.removeAttribute('data-coomi-dark-theme')
      return getComputedStyle(document.body).backgroundColor
    })
