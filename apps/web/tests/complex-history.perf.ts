...
import type { StreamChunk } from '@coomi/coomi-llm'
import {
  CallId,
...
import type { ReplayEntry, ReplayOverrideDoc } from '@coomi/coomi-llm-replay'
...
import type {} from '@coomi/coomi-session-title'
import {
  launchWebScaffold,
  seedSession,
  watchConsole,
...
async function startMutationProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Reflect.set(globalThis, '__coomiPerfMutationProbe', probe)
  })
}

async function stopMutationProbe(page: Page): Promise<MutationProbeResult> {
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__coomiPerfMutationProbe') as
    ...
    Reflect.deleteProperty(globalThis, '__coomiPerfMutationProbe')
...
async function startUserRenderProbe(
  ...
  await page.evaluate((expectedMarker) => {
    ...
    Reflect.set(globalThis, '__coomiPerfUserRenderProbe', probe)
  }, marker)
  await send.evaluate((button) => {
    button.addEventListener('click', (event) => {
      const probe = Reflect.get(globalThis, '__coomiPerfUserRenderProbe') as
...
async function stopUserRenderProbe(
  ...
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__coomiPerfUserRenderProbe') as
      ...
      | undefined
    Reflect.deleteProperty(globalThis, '__coomiPerfUserRenderProbe')
    if (
...
async function launchPerformanceWorld(
  ...
  try {
    if (options.replay === undefined) {
      ...
      replayDir = await mkdtemp(join(tmpdir(), 'coomi-web-perf-replay-'))
