...
// The model adapter is coomi-llm-replay with override sidecars: `hang` (+ a
...
import { deriveReplayScript, parseSessionLog } from '@coomi/coomi-llm-replay'
import type { ReplayOverrideDoc } from '@coomi/coomi-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(buildOverride?: (sidecarHome: string) => ReplayOverrideDoc): Promise<void> {
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'coomi-web-e2e-sidecar-'))
  ...
  it.skipIf(MODE === 'record')('cancels a hung stream deterministically via the readyFile marker', async () => {
    ...
    await expect.poll(
      () => page.getByRole('status').filter({ hasText: /^Coomi / }).isVisible(),
      { timeout: 10_000 },
    ).toBe(true)
