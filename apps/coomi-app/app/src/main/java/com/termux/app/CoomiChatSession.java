...
import static com.termux.app.CoomiActivity.EXTRA_ROUTE;
import static com.termux.app.CoomiActivity.EXTRA_SESSION_ID;
import static com.termux.app.CoomiActivity.EXTRA_PREFILL_DRAFT;
import static com.termux.app.CoomiActivity.EXTRA_RETURN_TO_SETUP;

/** Owns the single chat WebView and bridge independently of any Activity lifetime. */
public final class CoomiChatSession extends ContextWrapper {
    private static CoomiChatSession sInstance;
    private WeakReference<CoomiActivity> mActivity = new WeakReference<>(null);
    private final MutableContextWrapper mWebContext;
    private Intent mIntent = new Intent();
    private boolean mFloating;
    private boolean mBoundRequested;
    ...
    public static CoomiChatSession peek() { return sInstance; }

    private CoomiChatSession(Context context) {
        super(context);
        mWebContext = new MutableContextWrapper(context);
        mWebView = new WebView(mWebContext);
        configureWebView();
    }

    public WebView webView() { return mWebView; }
    public boolean isFloating() { return mFloating; }
    public boolean isLoaded() { return mPageFinished; }
    public boolean hasActivity() { return activity() != null; }
    public boolean hasPendingRequest() { return mRequestAction != null || mRequestActivity.get() != null; }

    private CoomiActivity activity() {
        CoomiActivity host = mActivity.get();
        return host == null || host.isDestroyed() || host.isFinishing() ? null : host;
    }

    public void attach(CoomiActivity host, ViewGroup container, Intent intent) {
        mActivity = new WeakReference<>(host);
        mIntent = new Intent(intent);
        detachWebView();
        mWebContext.setBaseContext(host);
        container.addView(mWebView, new ViewGroup.LayoutParams(-1, -1));
    ...
    public void detach(CoomiActivity host, boolean changingConfigurations) {
        if (mActivity.get() != host) return;
        mActivity.clear();
        if (!mFloating) detachWebView();
        if (mWebContext.getBaseContext() == host) mWebContext.setBaseContext(getApplicationContext());
        if (!changingConfigurations && !mFloating && mRequestAction == null
            && mRequestActivity.get() == null) destroy();
    }
    ...
    public void setFloating(boolean floating) {
        mFloating = floating;
        evaluateJavascript("window.__coomiFloating=" + floating
            + ";window.dispatchEvent(new CustomEvent('coomi:floating-state',{detail:" + floating + "}))");
    }

    public void moveActivityToBackground() {
        CoomiActivity host = activity();
        if (host != null) host.moveTaskToBack(true);
        mActivity.clear();
    }

    public void restoreFullscreen() {
        Intent intent = new Intent(this, CoomiActivity.class);
    ...
    public void closeFloating() {
        setFloating(false);
        CoomiActivity host = activity();
        if (host != null) host.attachSession();
        else if (mWebView != null) mWebView.evaluateJavascript(
            "window.dispatchEvent(new Event('coomi:flush-persistence'))",
            value -> { if (!mFloating && !hasActivity()) destroy(); });
    }

    public void flush() { evaluateJavascript("window.dispatchEvent(new Event('coomi:flush-persistence'))"); }

    private void runOnUiThread(Runnable action) {
        if (Looper.myLooper() == Looper.getMainLooper()) { if (!mDestroyed) action.run(); }
        else mHandler.post(() -> { if (!mDestroyed) action.run(); });
    }

    private Intent getIntent() { return mIntent; }
    ...
    private void withRequestActivity(Consumer<Activity> action) {
        runOnUiThread(() -> {
            if (mRequestAction != null || mRequestActivity.get() != null) {
                Toast.makeText(this, "请先完成当前系统操作", Toast.LENGTH_SHORT).show();
                return;
            }
            mRequestAction = action;
            try {
                startActivity(new Intent(this, CoomiChatRequestActivity.class));
            } catch (RuntimeException error) {
                mRequestAction = null;
                cancelPendingRequest();
                Toast.makeText(this, "无法打开系统窗口：" + error.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }

    void runRequest(CoomiChatRequestActivity host, boolean recreated) {
        mRequestActivity = new WeakReference<>(host);
        Consumer<Activity> action = mRequestAction;
        mRequestAction = null;
        if (action != null) {
            try { action.accept(host); }
            catch (RuntimeException error) {
                cancelPendingRequest();
                Toast.makeText(this, "无法打开系统窗口：" + error.getMessage(), Toast.LENGTH_LONG).show();
                host.finish();
            }
        } else if (!recreated) host.finish();
    }

    void releaseRequest(CoomiChatRequestActivity host, boolean changingConfigurations, boolean delivered) {
        if (mRequestActivity.get() == host) mRequestActivity.clear();
        if (!changingConfigurations && !delivered) cancelPendingRequest();
    }

    private void cancelPendingRequest() {
        if (mPendingImportRequestId != null) {
            emitFilesImported(new JSONArray(), mPendingImportRequestId);
            mPendingImportRequestId = null;
        }
        if (mPendingExportRequestId != null) {
            String id = mPendingExportRequestId;
            clearPendingExport();
            emitFileExported(id, null);
        }
        mPendingImageBytes = null;
    }

    private void startActivityForResult(Intent intent, int requestCode) {
        withRequestActivity(host -> host.startActivityForResult(intent, requestCode));
    }
    ...
    private void start() {
        ...
        startService(intent);
        mBoundRequested = bindService(intent, mConnection, Context.BIND_AUTO_CREATE);
        if (!mBoundRequested) showFailure(getString(R.string.coomi_engine_exited), null);
    }
    ...
    private void ensureEngineRunning() {
        ...
        mCoomiService.getEngineStatus(status -> {
            if (mDestroyed || mCoomiService == null) return;
    ...
    public void retryStart() {
        mStartRequested = false;
        mAutomaticRecoveryAttempts = 0;
        if (CoomiDemo.isEnabled()) { startDemo(); return; }
        showLoading(getString(R.string.coomi_engine_starting));
        if (mBoundRequested) ensureEngineRunning(); else start();
    }
    ...
    private void onEngineReady(int port) {
        if (mDestroyed || mPageLoaded) return;
        mPageLoaded = true;
        ...
        runOnUiThread(() -> {
            ...
            if (prefill != null && !prefill.isEmpty()) {
                mHandler.postDelayed(() -> evaluateJavascript(
                    "window.dispatchEvent(new CustomEvent('coomi:prefill-draft',{detail:{sessionId:window.__coomiSessionId||'',text:" + JSONObject.quote(prefill) + "}}))"), 800);
            }
        });
    }
    ...
    private void configureWebView() {
        ...
        mWebView.setWebViewClient(new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                if ("/__coomi_appearance/chat-background".equals(request.getUrl().getPath())) {
                    try {
                        String mimeType = CoomiTheme.backgroundMimeType(
                            CoomiChatSession.this, CoomiTheme.SURFACE_CHAT);
                ...
                return CoomiDemo.serve(CoomiChatSession.this, request.getUrl());
            }
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                mPageFinished = true;
                CoomiActivity host = activity();
                if (host != null) host.showSessionState(null, false, true);
    ...
    private void showLoading(String text) { showState(text, false); }

    private void showFailure(String message, String detail) {
        // 启动失败自动入队一条 startup_failure 反馈（去重；用户可在诊断页授权补传）。
        final String reason = detail == null || detail.isEmpty() ? message : detail;
        new Thread(() -> app.coomi.FeedbackManager.enqueueNativeError(
            CoomiChatSession.this, "startup_failure", "引擎启动失败", reason, null),
            "coomi-feedback-startup").start();
        showState(message, true);
    }

    private void showState(String message, boolean failed) {
        runOnUiThread(() -> {
            mLoadingMessage = message;
            mLoadingFailed = failed;
            CoomiActivity host = activity();
            if (host != null) host.showSessionState(message, failed, mPageFinished);
        });
    }
    ...
    public void openDashboard() {
        flush();
        Intent intent;
        if (mIntent.getBooleanExtra(EXTRA_RETURN_TO_SETUP, false)) {
            intent = new Intent(this, CoomiSetupActivity.class);
            intent.putExtra(CoomiSetupActivity.EXTRA_START_STEP, CoomiConstants.STEP_AUTH);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        } else {
            intent = new Intent(this, CoomiDashboardActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
        }
        startActivity(intent);
        CoomiActivity host = activity();
        if (host != null) host.overridePendingTransition(
    ...
    private final class AndroidBridge {
        ...
        public void openFloatingWindow() {
            runOnUiThread(() -> {
                CoomiActivity host = activity();
                if (host != null && !mFloating) host.openFloatingWindow();
            });
        }
        ...
        public String getDiagnostics() {
            return app.coomi.CoomiFeedbackClient.diagnostics(CoomiChatSession.this).toString();
        }

        /** 原生上报报错反馈：统一走 FeedbackManager（脱敏终检 + 环境补齐 + Outbox 兜底），
         *  后台线程执行，绕过 WebView 跨域/CORS 限制。
         *  完成回调 window.__coomiFeedbackResult(callbackId, {ok, status})。 */
        @JavascriptInterface
        public void sendFeedback(String json, String callbackId) {
            new Thread(() -> {
                ...
                try {
                    ...
                    result = app.coomi.FeedbackManager.submit(CoomiChatSession.this, payload);
        ...
        /** 前端上报的运行时异常：弹原生「检测到异常」对话框，用户一键授权后立即采集上传。 */
        @JavascriptInterface
        public void reportError(String type, String title, String detail) {
            runOnUiThread(() -> {
                CoomiActivity host = activity();
                if (host != null) host.showReportErrorDialog(type, title, detail);
            });
        }
        ...
        public String getThemeMode() {
            return CoomiTheme.getMode(CoomiChatSession.this);
        }
        ...
        public void setThemeMode(String mode) {
            if (CoomiTheme.isCustomEnabled(CoomiChatSession.this)) return;
            CoomiTheme.setMode(CoomiChatSession.this, mode);
            runOnUiThread(() -> {
                applyThemeToWebView();
                CoomiActivity host = activity();
                if (host != null) CoomiTheme.applySystemBars(host);
        ...
        public void openFile(String path) {
            runOnUiThread(() -> {
                try {
                    ...
                    if (!file.isFile()) {
                        Toast.makeText(CoomiChatSession.this, "文件不存在：" + path, Toast.LENGTH_SHORT).show();
                        return;
                    }
                    android.net.Uri uri = androidx.core.content.FileProvider.getUriForFile(
                        CoomiChatSession.this, getPackageName() + ".fileprovider", file);
        ...
        public void installApk(String url, String version) {
            app.coomi.UpdateChecker.downloadAndInstall(CoomiChatSession.this, url, version);
        }
        ...
        public void saveImageData(String dataUrl, String fileName) {
            ...
            if (bytes == null) {
                Toast.makeText(CoomiChatSession.this, "图片数据无效", Toast.LENGTH_SHORT).show();
                return;
            }
            final String mime = mimeFromDataUrl(dataUrl);
            withRequestActivity(host -> {
                if (Build.VERSION.SDK_INT >= 29) {
                        ...
                        .setItems(new String[]{"保存到相册", "保存到下载目录"}, (dialog, which) -> {
                            new Thread(() -> {
                                ...
                                runOnUiThread(() -> Toast.makeText(
                                    CoomiChatSession.this, ok ? "已保存" : "保存失败", Toast.LENGTH_SHORT).show());
                            }).start();
                        })
                    ...
                    host.startActivityForResult(intent, REQUEST_SAVE_IMAGE);
                }
            });
        }
        ...
        public boolean getDigitalLifeEnabled() {
            return CoomiTheme.isDigitalLifeEnabled(CoomiChatSession.this);
        }
    ...
    void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode != Activity.RESULT_OK || data == null) {
            ...
            if (requestCode == REQUEST_SAVE_IMAGE) mPendingImageBytes = null;
            return;
        }
        if (requestCode == REQUEST_IMPORT_FILES) {
            ...
            new Thread(() -> {
                ...
                runOnUiThread(() -> Toast.makeText(
                    CoomiChatSession.this, saved ? "已保存" : "保存失败", Toast.LENGTH_SHORT).show());
    ...
    public void onBackPressed() {
        if (mWebView == null || !mPageLoaded) { openDashboard(); return; }
        mWebView.evaluateJavascript(
            "typeof window.__coomiHandleSystemBack==='function' && window.__coomiHandleSystemBack()",
            value -> { if (!"true".equals(value)) openDashboard(); });
    }
    ...
    private void destroy() {
        ...
        if (mBoundRequested) unbindService(mConnection);
        mBoundRequested = false;
        mCoomiService = null;
        mActivity.clear();
        mRequestAction = null;
        CoomiChatRequestActivity request = mRequestActivity.get();
        if (request != null) request.finish();
        mRequestActivity.clear();
        detachWebView();
        mWebContext.setBaseContext(getApplicationContext());
        mWebView.removeJavascriptInterface("CoomiAndroid");
