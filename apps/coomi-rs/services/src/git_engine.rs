...
fn parse_porcelain_v2(raw: &str, status: &mut GitStatus) {
    for entry in raw.split('\0') {
        ...
        match marker {
            ...
            '1' => {
                // porcelain v2: 1 <XY> <sub> <mH> <mI> <mW> <hH> <hI> <path> = 9 字段
                let Some(fields) = split_fields(line, 9) else {
                    continue;
                };
                let xy = fields[1];
                push_classified(status, xy, fields[8], None);
            }
            '2' => {
                // 重命名/复制：2 <XY> <sub> <mH> <mI> <mW> <hH> <hI> <X><score> <path> = 10 字段
                let Some(fields) = split_fields(line, 10) else {
                    continue;
                };
                push_classified(status, fields[1], fields[9], None);
            }
            'u' => {
                // 冲突：u <XY> <sub> <m1> <m2> <m3> <mW> <h1> <h2> <h3> <path> = 11 字段
                let Some(fields) = split_fields(line, 11) else {
                    continue;
                };
