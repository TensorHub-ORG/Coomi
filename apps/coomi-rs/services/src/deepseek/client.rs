...
use super::pow::{PowChallenge, pow_header_json, solve_challenge};

const API_BASE: &str = "https://chat.deepseek.com";
const USER_AGENT: &str = "DeepSeek/2.5.0 Android";
const SMS_SEND_TARGET: &str = "/v0/users/create_sms_verification_code";
const SMS_LOGIN_TARGET: &str = "/v0/users/login_by_mobile_sms";
...
pub async fn login(client: &Client, account: &str, password: &str) -> Result<LoginResult> {
    ...
    login_request(client, "/api/v0/users/login", body, None).await
}

/// 请求手机号短信验证码。字段与 DeepSeek 2.5.0 的
...
pub async fn send_sms_code(client: &Client, mobile: &str, area_code: &str) -> Result<()> {
    let mobile = mobile.trim().replace(' ', "").replace('-', "");
    ...
    let guest_pow = guest_pow_header(client, SMS_SEND_TARGET).await?;
    let resp = client
        .post(format!(
            "{API_BASE}/api/v0/users/create_sms_verification_code"
        ))
        .header("Content-Type", "application/json")
        .header("User-Agent", USER_AGENT)
        .header("X-DS-Guest-PoW-Response", guest_pow)
        .json(&body)
        .send()
        .await
        .context("发送验证码请求失败")?;
...
async fn login_request(
    ...
    if let Some(target) = guest_target {
        request = request.header(
            ...
            guest_pow_header(client, target).await?,
        );
    }
    let resp = request.json(&body).send().await.context("登录请求失败")?;
    let status = resp.status();
...
/// 创建新会话：与 DeepSeek Android 2.5.0 一致，POST /api/v0/chat_session/create。
...
pub async fn chat_completion(
    ...
    let solution = super::pow::solve_challenge(&pow)?;
    let pow_json = pow_header_json(&solution, "/api/v0/chat/completion")?;
    let pow_header = base64::engine::general_purpose::STANDARD.encode(pow_json.as_bytes());
    let body = serde_json::json!({
...
pub async fn create_pow_challenge(client: &Client) -> Result<PowChallenge> {
    ...
    ensure_business_success(&body, "创建 challenge")?;
    parse_challenge(&body, false)
}
...
fn encode_header_json(value: &serde_json::Value) -> Result<String> {
    let bytes = serde_json::to_vec(value).context("编码 PoW 响应失败")?;
    Ok(base64::engine::general_purpose::STANDARD.encode(bytes))
}

fn parse_challenge(body: &serde_json::Value, guest: bool) -> Result<PowChallenge> {
    let challenge = if guest {
        body.pointer("/data/biz_data/guest_challenge")
            .or_else(|| body.pointer("/data/guest_challenge"))
            .or_else(|| body.get("guest_challenge"))
    } else {
        body.pointer("/data/biz_data/challenge")
            .or_else(|| body.pointer("/data/challenge"))
            .or_else(|| body.get("challenge"))
    }
    .ok_or_else(|| anyhow!("challenge 响应缺少挑战数据"))?;
    serde_json::from_value(challenge.clone()).context("解析 challenge 结构失败")
}

async fn guest_pow_header(client: &Client, target_path: &str) -> Result<String> {
    let response = client
        .post(format!("{API_BASE}/api/v0/users/create_guest_challenge"))
        .header("Content-Type", "application/json")
        .header("User-Agent", USER_AGENT)
        .json(&serde_json::json!({"target_path": target_path}))
        .send()
        .await
        .context("创建 DeepSeek 访客挑战失败")?;
    let status = response.status();
    let body: serde_json::Value = response.json().await.context("解析访客挑战失败")?;
    if !status.is_success() {
        return Err(anyhow!("创建访客挑战失败 HTTP {status}: {body}"));
    }
    ensure_business_success(&body, "创建访客挑战")?;
    let solution = solve_challenge(&parse_challenge(&body, true)?)?;
    encode_header_json(&serde_json::json!({
        "salt": solution.salt,
...
pub async fn read_chat_stream(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        for line in s.split('\n') {
            ...
            let value: serde_json::Value = serde_json::from_str(json).context("解析 SSE data")?;
            // text_delta
...
mod tests {
    ...
    #[test]
    fn pow_headers_are_base64_encoded_json() {
        let encoded = encode_header_json(&serde_json::json!({"salt": "s", "answer": 12.0}))
        ...
        assert_eq!(
            ...
            serde_json::json!({"salt": "s", "answer": 12.0})
        );
    }

    #[test]
    fn challenge_parser_accepts_current_biz_data_shapes() {
        let challenge = serde_json::json!({
            "algorithm": "DeepSeekHashV1", "challenge": "c", "salt": "s",
            "difficulty": 20, "signature": "sig"
        });
        let chat = serde_json::json!({"data": {"biz_data": {"challenge": challenge}}});
        let guest = serde_json::json!({"data": {"biz_data": {"guest_challenge": challenge}}});
        assert_eq!(parse_challenge(&chat, false).unwrap().salt, "s");
        assert_eq!(parse_challenge(&guest, true).unwrap().signature, "sig");
    }
}
