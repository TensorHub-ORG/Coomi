/**
 * `coomi plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `coomi.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `coomi.bundle` joins the layer stack; a
 ...
 * gained its `coomi.bundle` declaration in a newer version.
 * @module @coomi/coomi/plugin
 */
 ...
 * @returns true when the package manifest declares `coomi.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.coomi?.bundle?.patch !== undefined
}

/**
 * Reconcile `coomi.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `coomi.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.coomi?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no coomi.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (coomi-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.coomi = { ...after.coomi, profile: { ...after.coomi?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `coomi` was invoked from.
...
/**
 * Run one `coomi plugin` invocation: init if needed, forward to pnpm, reconcile.
