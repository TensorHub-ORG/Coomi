...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@coomi/coomi-loader-smoke'

const coomiBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"coomi-test: never-dispose ready", b"coomi-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'coomi-headless-shutdown-'))
  try {
    const home = join(cwd, '.coomi')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'coomi-profile-headless',
      private: true,
      dependencies: {},
      coomi: { profile: { bundles: ['@coomi/coomi-base', '@coomi/coomi-headless'] } },
    }, undefined, 2))
    ...
    if (result.timedOut) {
      throw new Error(`coomi headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`coomi headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('coomi: observing at ')
    expect(output).toContain('coomi-test: never-dispose ready')
    expect(output).toContain('coomi-test: never-dispose started')
