 ...
 * the REAL shipped bundle layers (coomi-base + coomi-web-app resolved from the
...
import { entryListSchema } from '@coomi/cordis-plugin-include'
import { evaluate } from '@coomi/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@coomi/coomi-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL coomi-base/coomi-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'coomi-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@coomi/coomi-base', '@coomi/coomi-web-app'])
    const profile = loadProfile('coomi', 'web', anchor, home)
    ...
    for (const name of ['@coomi/coomi-pwsh-sandbox', '@coomi/coomi-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'coomi-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@coomi/coomi-base'])
    const profile = loadProfile('coomi', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
