...
import { parseCoomiArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseCoomiArgs(argv, '1.2.3')
...
describe('parseCoomiArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@coomi/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@coomi/cordis'] })
