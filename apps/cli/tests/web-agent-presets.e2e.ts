...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@coomi/coomi-app-boot'
...
import type { PatchOptions } from '@coomi/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { settingsNamespace } from '@coomi/coomi-settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@coomi/coomi-agent-presets'
import { applyChildComposition, childSessionMeta } from '@coomi/coomi-subagent'
import { CallId } from '@coomi/coomi-llm'
import type {} from '@coomi/coomi-compaction-basic'
...
import type {} from '@coomi/coomi-token-meter'

const CONFIG_DIR = fileURLToPath(new URL('../config/', import.meta.url))
const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the coomi-base and coomi-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...loadOverlayPatches('coomi-test', BASE_PATCH),
    ...loadOverlayPatches('coomi-test', WEB_PATCH),
    // The settings row defaults to `$COOMI_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $COOMI_HOME. Unpinned, this
    // file writes the developer's own `~/.coomi/storages/` — and then reads it
    ...
    // developer's own `~/.coomi/.preset` cannot change this test's outcome.
  ...
  return await boot('coomi-test', rootConfig, patches, (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'coomi-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'coomi-preset-skill-proj-'))
    await mkdir(join(proj, '.coomi', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.coomi', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient coomi home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['coomi-badge'])
      ...
      expect(scoped).toContain('coomi-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by coomi')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('coomi-badge')
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    const root = await mkdtemp(join(tmpdir(), 'coomi-product-presets-'))
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@coomi/coomi-subagent-codex' },
        { id: 'subagent-claude-code', name: '@coomi/coomi-subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<coomiHome>/.agent-presets` itself. `$COOMI_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'coomi-preset-derived-'))
    previousHome = process.env.COOMI_HOME
    process.env.COOMI_HOME = home
    await mkdir(join(home, '.agent-presets', 'derived-mine'), { recursive: true })
    await writeFile(
      join(home, '.agent-presets', 'derived-mine', 'agent.cordis.yml'),
      '- id: tool-todo\n  name: \'@coomi/coomi-tool-todo\'\n  config:\n    allowParallelInProgress: true\n',
    )
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'coomi-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.COOMI_HOME
    else process.env.COOMI_HOME = previousHome
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'coomi-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'coomi-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
