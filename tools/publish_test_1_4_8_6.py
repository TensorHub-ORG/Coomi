"""Publish the signed 1.4.8-test.6 APK to the existing Android test channel.
...
def website_update(source):
    ...
    if f'v{VERSION} 更新说明' in source:
        return source
    if marker not in source or 'Coomi-Android-arm64-v1.4.8-test.5.apk' not in source:
        raise RuntimeError('Website baseline does not contain expected test.5 entry')
    notes = (ROOT / 'docs/releases/v1.4.8-test.6.md').read_text(encoding='utf-8')
    items = [line[2:] for line in notes.splitlines() if line.startswith('- ')]
    block = '\n<h2 class="changelog-title">v'+VERSION+' 更新说明【测试】</h2>\n<p class="changelog-sub">2026-09-14 发布</p>\n<ul class="changelog-list">\n'
    block += '\n'.join('<li>'+html.escape(item)+'</li>' for item in items) + '\n</ul>\n'
    source = source.replace('Coomi-Android-arm64-v1.4.8-test.5.apk', NAME)
...
def main():
    ...
    if not args.inspect and not args.prepare_site:
        verify_bundle(APK)
    client = connect()
    try:
        ...
        if args.prepare_site:
            ...
            for name in ['index.html', 'index.remote.html']:
                (ROOT/'server/site'/name).write_text(updated, encoding='utf-8', newline='\n')
        ...
        notes = (ROOT/'docs/releases/v1.4.8-test.6.md').read_text(encoding='utf-8').lstrip('# ')
        manifest = dict(versionCode=CODE, version=VERSION, file=NAME, channel='test', platform='android',
                        arch='arm64-v8a', minAndroid='7.0', date='2026-09-14', size=APK.stat().st_size,
