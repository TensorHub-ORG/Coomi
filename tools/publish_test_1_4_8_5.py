"""Publish the signed 1.4.8-test.5 APK to the existing Android test channel.
...
def website_update(source):
    ...
    if f'v{VERSION} 更新说明' in source:
        return source
    if marker not in source or 'Coomi-Android-arm64-v1.4.8-test.4.apk' not in source:
        raise RuntimeError('Website baseline does not contain expected test.4 entry')
    notes = (ROOT / 'docs/releases/v1.4.8-test.5.md').read_text(encoding='utf-8')
    ...
    source = source.replace('Coomi-Android-arm64-v1.4.8-test.4.apk', NAME)
...
def main():
    ...
    try:
        ...
        notes = (ROOT/'docs/releases/v1.4.8-test.5.md').read_text(encoding='utf-8').lstrip('# ')
