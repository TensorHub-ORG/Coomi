...
import { boot, resolveConfigPath } from '@coomi/coomi-app-boot'
import { recordFeedback } from '@coomi/coomi-command-feedback'
import { runFixtureTurn } from '@coomi/coomi-loader-smoke'
...
process.env.COOMI_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.COOMI_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
