...
import { defineAcpSnapshotSuite, type Scenario, type SnapshotSuiteOptions } from '@coomi/coomi-acp-snapshot'
import { resolvePwshPath } from '@coomi/coomi-pwsh-local'
import { decodeStorageRecord } from '@coomi/coomi-session'

/**
 * The acp-agent example's snapshot suite: the scenario table for
 * `coomi-acp-snapshot`'s suite factory, which owns every compare/guard mechanic
...
// The coomi-acp-demo bin (the demo:acp entry), this example's cordis.yml, and
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown COOMI_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { COOMI_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      COOMI_PERMISSION_MODE: 'read-only',
      COOMI_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // (XXX(concurrent-subagents) in coomi-llm-replay).
  ...
  // published-handle disposal failure.
  {
    ...
    env: { COOMI_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { COOMI_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.COOMI_SNAPSHOT),
  hasPwsh,
})
