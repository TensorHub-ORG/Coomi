 ...
 * the REAL `coomi-jsonrpc-agent` runtime (per `COOMI_EXAMPLE_MODE`) through the
 * REAL `@coomi/coomi-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `COOMI_SNAPSHOT=record`
 * re-records against the live API; `COOMI_SNAPSHOT=refresh` replays committed
...
import { resolveExampleLaunch } from '@coomi/coomi-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@coomi/coomi-sdk-client'
...
const mode = process.env.COOMI_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  {
    name: 'bash-tool',
    prompt: 'Run this exact command with your bash tool, then reply with its stdout only: echo coomi-sdk-proof-7391',
  ...
  },
  {
    ...
    environment: { COOMI_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    COOMI_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    COOMI_SESSION_ROOT: sessionsRoot,
    COOMI_CWD: cwd,
    COOMI_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      COOMI_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { COOMI_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
