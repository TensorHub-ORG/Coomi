...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/coomi/coomi-desktop' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/coomi/coomi-desktop/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
/**
 * The Coomi wordmark, inlined so its `currentColor` fills follow the active
...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="coomi-wordmark" ')
 ...
 */
const siteStyle = `
.coomi-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.coomi-wordmark { display: block; height: 22px; width: auto; color: var(--vp-c-text-1); }
.coomi-tag {
  display: inline-flex;
  align-items: center;
...
function siteTitle(previewTag: string): string {
  return `<span class="coomi-lockup">${wordmark}<span class="coomi-tag">${previewTag}</span></span>`
}
...
export default withMermaid({
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'coomi-desktop-doc-projector',
