...
function checkManifest(
  ...
  if (owner.packageName === '@coomi/coomi-invariants') return
  if (manifest.peerDependencies?.['@coomi/coomi-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@coomi/coomi-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@coomi/coomi-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@coomi/coomi-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@coomi/coomi-invariants'
