...
const CLIENT_NOTES: readonly string[] = [
  ...
  PRIORITY_NOTE,
  'You cannot `import` anything, so the design-system components are out of reach: build markup with `React.createElement` and ship CSS through `styles.insert(css)`. Use the theme CSS variables (`var(--coomi-alias-bg-layer-1)`, `var(--coomi-alias-label-primary)`, …) instead of literal colors, or your contribution breaks in the other color scheme.',
...
/** Drop the `@coomi/coomi-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@coomi/coomi-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @coomi/coomi-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
