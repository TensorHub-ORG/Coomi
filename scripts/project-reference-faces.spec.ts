...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'coomi-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@coomi/coomi-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@coomi/coomi-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@coomi/coomi-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@coomi/coomi-client-consumer' })
