...
- id: sessions
  ...
  config:
    root: !!js process.env.COOMI_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@coomi/coomi-code-runtime-worker-thread'
- id: subagents
  name: '@coomi/coomi-subagent'
- id: subagent-spawn-in-process
  name: '@coomi/coomi-subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
  ...
  config:
    provider: spawn
- id: workflow-engine
  name: '@coomi/coomi-workflow-worker-thread'
  config:
    provider: spawn
- id: workflow-tool
  name: '@coomi/coomi-tool-workflow'
- id: cordis-host-runner
  name: '@coomi/coomi-cordis-host-runner'
- id: cordis-tool
  name: '@coomi/coomi-tool-cordis'
"""
...
def smoke_sdk_default(base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="coomi-sdk-default-") as temporary:
...
def smoke_sdk_custom(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="coomi-sdk-custom-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_minimal(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="coomi-sdk-minimal-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="coomi-sdk-snapshot-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="coomi-direct-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
        environment = {
            **os.environ,
            "COOMI_CORDIS_CONFIG": str(cordis),
            "COOMI_SESSION_ROOT": str(sessions),
            "COOMI_CWD": str(root),
