...
SDK_DISTRIBUTION = "coomi-desktop-sdk"
RUNTIME_DISTRIBUTION = "coomi-desktop-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="coomi-python-release-") as temporary:
        ...
        if args.package == "sdk":
            ...
            environment = {"COOMI_RUNTIME_PLATFORM_TAG": platform_tag}
            expected = output_dir / f"coomi_harness_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"coomi-desktop-runtime-bin==[^"]+"',
        f'"coomi-desktop-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def stage_runtime(destination: Path, version: str, executable: Path, executable_name: str) -> None:
    ...
    runtime_dir = destination / "src" / "coomi_harness_runtime" / "runtime"
...
def verify_wheel(
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        runtime_files = [
            name for name in archive.namelist() if "/runtime/coomi-jsonrpc-agent-pkg-" in name
        ]
        if package == "runtime":
