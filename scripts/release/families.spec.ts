...
describe('release families', () => {
  it('names one tag for the whole coomi family and one per vendored package', () => {
    const coomi = releaseFamily('coomi')
    const vendor = releaseFamily('vendor')
    const cli = member('apps/cli', '@coomi/coomi')
    const cordis = { ...member('vendor/cordis', '@coomi/cordis'), version: '4.0.1' }

    expect(coomi.tagFor(cli)).toBe('coomi-v0.0.1')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const coomi = releaseFamily('coomi')
    const members = [member('apps/cli', '@coomi/coomi'), { ...member('apps/web', '@coomi/coomi-web-frontend'), version: '0.0.2' }]

    expect(() => { coomi.verifyVersions(members) }).toThrow(/must share one version/)
    expect(() => { coomi.verifyVersions([members[0]!]) }).not.toThrow()
  })

  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@coomi/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@coomi/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const coomi = releaseFamily('coomi')
    const members = [
      member('packages/a/consumer', '@coomi/coomi-consumer', { dependencies: { '@coomi/coomi-library': 'workspace:^' } }),
      member('packages/a/library', '@coomi/coomi-library'),
      member('packages/a/zebra', '@coomi/coomi-zebra'),
    ]

    expect(coomi.publishOrder(members).map(entry => entry.name)).toEqual([
      '@coomi/coomi-library',
      '@coomi/coomi-consumer',
      '@coomi/coomi-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const coomi = releaseFamily('coomi')
    const members = [
      member('packages/a/left', '@coomi/coomi-left', { dependencies: { '@coomi/coomi-right': 'workspace:^' } }),
      member('packages/a/right', '@coomi/coomi-right', { dependencies: { '@coomi/coomi-left': 'workspace:^' } }),
    ]

    expect(() => { coomi.publishOrder(members) }).toThrow(/dependency cycle/)
  })

  it('applies the harness payload policy to coomi and keeps upstream payloads for vendored packages', () => {
    const coomi = releaseFamily('coomi')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@coomi/coomi-library')
    const vendored = member('vendor/cordis', '@coomi/cordis')

    expect(() => { coomi.validatePayload(harness, ['package/lib/index.js', 'package/src/index.ts']) })
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('coomi').installedEntry).toEqual({ packageName: '@coomi/coomi', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@coomi/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@coomi/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@coomi/coomi-library', { files: ['lib/index.js'] }),
