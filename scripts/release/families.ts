 ...
 * owns: `coomi` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@coomi/coomi-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@coomi/')) throw new Error(`${normalized} must name an @coomi package`)
...
class coomiFamily extends ReleaseFamily {
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`coomi release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@coomi/coomi', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@coomi/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new coomiFamily(), new VendorFamily()]
}
