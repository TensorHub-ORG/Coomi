...
const SESSION_PACKAGE = '@coomi/coomi-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@coomi/coomi-session/types'
 ...
 * top-level declaration (in `@coomi/coomi-session`) and any declaration
 * merge inside a `declare module '@coomi/coomi-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @coomi/coomi-session/known-event-types',
    ' */',
    '',
    '/**',
