...
const vendoredPackages = new Set([
  ...
  '@coomi/cosmokit',
  '@coomi/schemastery',
  '@coomi/cordis-plugin-loader',
  '@coomi/cordis-plugin-include',
  '@coomi/cordis-plugin-group',
  '@coomi/cordis-plugin-timer',
  '@coomi/cordis-plugin-hmr',
  '@coomi/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@coomi/node-addon-landlock-run',
  '@coomi/node-addon-landlock-run-linux-arm64',
  '@coomi/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@coomi/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/coomi-desktop/coomi-desktop.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/coomi/coomi-desktop.git'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@coomi/coomi': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@coomi/coomi-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  // Profile bundles publish their coomi.bundle.patch layer beside the lib.
  '@coomi/coomi-base': ['cordis.patch.yml'],
  '@coomi/coomi-web-app': ['cordis.patch.yml'],
  '@coomi/coomi-headless': ['cordis.patch.yml'],
  '@coomi/coomi-client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@coomi/coomi-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@coomi/coomi-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@coomi/coomi-skill-badge': ['assets'],
  '@coomi/coomi-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function expectedCoomiPackageFiles(manifest: PackageManifest): readonly string[] {
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the coomi family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@coomi/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@coomi/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@coomi/coomi-')) {
    const peer = manifest.peerDependencies?.['@coomi/cordis']
    ...
    const expectedFiles = expectedCoomiPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the coomi release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:coomi` and its publication.
