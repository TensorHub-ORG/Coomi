 ...
 * silently changes composition. Example configs and the coomi Web composition
...
// These example files are overlays consumed by the built coomi app, so their bare
...
const CHOOSER_PACKAGE = '@coomi/coomi-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@coomi/coomi-host-directory-picker-native',
  '@coomi/coomi-host-directory-picker-browse',
  '@coomi/coomi-client-ui-directory-picker-browse',
  '@coomi/coomi-client-ui-directory-picker-native',
]
 ...
 * `coomi.client` block, and the node half of a surface plugin is an empty
 ...
 * `coomi.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    const manifest = readManifest(manifestPath) as PackageManifest & {
      ...
      coomi?: { client?: unknown }
    }
    const shipsClient = manifest.exports !== undefined && Object.hasOwn(manifest.exports, './client')
    const declaresClient = manifest.coomi?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no coomi.client, so its browser half is never served`
      : `${manifestPath}: declares coomi.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `COOMI_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@coomi/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@coomi/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // coomi app's own dependency surface — the profile module fallback mirrors it.
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `coomi` source
