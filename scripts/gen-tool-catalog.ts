...
import type { ToolSchema } from '@coomi/coomi-llm'
import AgentRegistry from '@coomi/coomi-agent'
import type { Agent } from '@coomi/coomi-agent'
import { createScope } from '@coomi/coomi-scope'
import SessionStore, { SessionId } from '@coomi/coomi-session'
import SessionProjectionRegistry from '@coomi/coomi-session-projection'
import SqliteSessionQueryEngine from '@coomi/coomi-session-query-sqlite'
import GoalService from '@coomi/coomi-goal'
import SystemPrompt from '@coomi/coomi-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@coomi/coomi-tools'
import LocalBashExecutor from '@coomi/coomi-bash-local'
import * as BashEnvPlugin from '@coomi/coomi-shell-env'
import { PwshLocalExecutor } from '@coomi/coomi-pwsh-local'
import LocalSubprocessRuntime from '@coomi/coomi-subprocess-local'
import LocalFileSystem from '@coomi/coomi-fs-local'
import { AttachmentStore } from '@coomi/coomi-attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@coomi/coomi-attachment'
import UserQuestionService from '@coomi/coomi-user-questions'
import PlanModeController from '@coomi/coomi-plan-mode'
import WebRuntime from '@coomi/coomi-web'
import * as WebSearchExa from '@coomi/coomi-web-search-exa'
import * as WebFetchLocal from '@coomi/coomi-web-fetch-http'
import SubagentRuntime from '@coomi/coomi-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@coomi/coomi-subagent'
import * as ToolSubagentControl from '@coomi/coomi-tool-subagent-control'
import * as ToolSubagentListAgents from '@coomi/coomi-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@coomi/coomi-tool-subagent-report'
import SkillRegistry from '@coomi/coomi-skill'
import * as SkillFileSystem from '@coomi/coomi-skill-filesystem'
import LocalJobRegistry from '@coomi/coomi-jobs-local'
import * as ToolAskUser from '@coomi/coomi-tool-ask-user'
import * as ToolBash from '@coomi/coomi-tool-bash'
import * as ToolPwsh from '@coomi/coomi-tool-pwsh'
import * as ToolBashPersistent from '@coomi/coomi-tool-bash-persistent'
import CordisHostRunner from '@coomi/coomi-cordis-host-runner'
import * as ToolCordis from '@coomi/coomi-tool-cordis'
import * as ToolFs from '@coomi/coomi-tool-fs'
import * as ToolFsSearch from '@coomi/coomi-tool-fs-search'
import * as ToolStrReplaceEditor from '@coomi/coomi-tool-str-replace-editor'
import TerminalSessionService from '@coomi/coomi-terminal'
import * as ToolPty from '@coomi/coomi-tool-terminal'
import * as ToolGoal from '@coomi/coomi-tool-goal'
import * as ToolSchedule from '@coomi/coomi-schedule'
import Lsp from '@coomi/coomi-lsp'
import * as ToolLsp from '@coomi/coomi-tool-lsp'
import * as ToolSkill from '@coomi/coomi-tool-skill'
import * as ToolSessionQuery from '@coomi/coomi-tool-session-query'
import * as ToolTasks from '@coomi/coomi-tool-jobs'
import * as ToolTodo from '@coomi/coomi-tool-todo'
import * as ToolSubagent from '@coomi/coomi-tool-subagent'
import * as ToolWeb from '@coomi/coomi-tool-web'
import VmWorkflowEngine from '@coomi/coomi-workflow-worker-thread'
import * as ToolRalph from '@coomi/coomi-tool-ralph'
import * as ToolWorkflow from '@coomi/coomi-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@coomi/coomi-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@coomi/coomi-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@coomi/coomi-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `COOMI_*` environment comes from `@coomi/coomi-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@coomi/coomi-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@coomi/coomi-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or coomi restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@coomi/coomi-tool-bash-persistent',
  ...
  {
    pkg: '@coomi/coomi-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@coomi/coomi-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@coomi/coomi-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@coomi/coomi-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@coomi/coomi-tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        coomiHome: resolve(root, '.tmp/tool-catalog/.coomi'),
  ...
  {
    pkg: '@coomi/coomi-tool-session-query',
  ...
  {
    pkg: '@coomi/coomi-tool-subagent-control',
  ...
  {
    pkg: '@coomi/coomi-tool-subagent-report',
