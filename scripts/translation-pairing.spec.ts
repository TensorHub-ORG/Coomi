...
function gitSupportsObjectFormat(format: 'sha256'): boolean {
  const root = mkdtempSync(join(tmpdir(), 'coomi-git-object-format-'))
  try {
...
describe('translation pairing snapshots', () => {
  it('stores exact uncommitted bytes for later recovery by object ID', () => {
    const root = mkdtempSync(join(tmpdir(), 'coomi-translation-pairing-'))
    try {
      ...
      expect(execFileSync('git', [
        '-C', root, 'rev-parse', `refs/coomi/translation-pairing/snapshots/${objectId}`,
  ...
  it('reads staged bytes independently of the working tree', () => {
    const root = mkdtempSync(join(tmpdir(), 'coomi-translation-pairing-index-'))
    try {
      execFileSync('git', ['init', '--quiet', root], {
        env: { ...process.env, GIT_DEFAULT_HASH: 'sha1' },
      })
...
describe('translation pairing switchers', () => {
  ...
  it('accepts only the canonical public URL for an absolute switcher', () => {
    ...
    const canonical = parseTranslationMarkdown(
      '[中文](https://github.com/coomi/coomi-desktop/blob/master/python/sdk/README.zh.md)',
    )
    const wrongPath = parseTranslationMarkdown(
      '[中文](https://github.com/coomi/coomi-desktop/blob/master/other/README.zh.md)',
    )
...
describe('translation scope discovery', () => {
  ...
  it.each([
    ...
    'python/sdk-runtime/src/coomi_harness_runtime/runtime/coomi-jsonrpc-agent-macos-arm64/README.md',
    'python/sdk-runtime/src/coomi_harness_runtime/runtime/node/README.md',
