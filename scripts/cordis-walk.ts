...
const MERGE_HEAD = /declare module ['"](?:@coomi\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@coomi/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@coomi/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@coomi/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@coomi/cordis'` Context merge
...
/**
 * Every event name a `declare module '@coomi/cordis'` Events merge declares in one
