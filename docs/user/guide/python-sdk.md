...
# export COOMI_MODEL=deepseek-v4-flash
# export COOMI_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Property | Value |
|---|---|
| System prompt | `COOMI_SYSTEM_PROMPT`, falling back to `You are a helpful software engineer assistant.` |
| Model in `minimal.py` | `--model`, then `COOMI_MODEL`, then `deepseek-v4-flash` |
...
| Session persistence | Uncompressed JSONL under `COOMI_SESSION_ROOT` |
