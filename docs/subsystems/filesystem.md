...
The optional filesystem capability has four parts: [coomi-fs](../../packages/fs/fs) owns `ctx.fs` and atomic text operations with optional guards, [coomi-fs-local](../../packages/fs/fs-local) implements local disk, [coomi-fs-observation-policy](../../packages/fs/fs-observation-policy) records observed presence or absence and adds freshness rules through events rather than a service, and [coomi-tool-fs](../../packages/fs/tool-fs) directly executes model-facing read/write/edit calls and renders windows. It is outside the agent-loop spine; alternate backends do not change policy or tool schemas.

`coomi-fs-observation-policy` is optional. Without it, the `FileSystem` Service Definition, a provider, and the `coomi-tool-fs` Consumer form the complete, unconstrained filesystem seam: `write` unconditionally creates or overwrites, and `edit` unconditionally replaces literal text. The policy plugin changes these operations by deciding the `fs/*` waterfalls. Removing it does not break the tool because the tool calls `ctx.fs` and dispatches events; it does not call policy methods. A deployment that loads `coomi-tool-fs` is expected to also load `coomi-fs-observation-policy` so the default behavior is read-before-write/edit.
...
`coomi-fs` owns three events the tool dispatches and the policy plugin listens for, so the emitter (`coomi-tool-fs`) and the listener (`coomi-fs-observation-policy`) share a vocabulary without the emitter depending on the policy plugin. They carry only `coomi-fs` vocabulary plus an opaque `object` actor — no model-facing concepts and no agent/session owner structure.
...
The policy plugin needs just enough execution context to derive the observed-state owner by narrowing the opaque `object` actor the `fs/*` events carry. `ToolExecution` has the required fields, so `coomi-tool-fs` passes its execution object through as the actor without making `coomi-fs-observation-policy` import the tool, agent, or session packages.

```ts type-equiv
/**
 * Minimal structural view of a tool execution the policy plugin needs to derive
 * an observed-state owner. `@coomi/coomi-tools`' `ToolExecution` contains
 ...
 * `FsObservationActor` without importing `coomi-tools`, `coomi-agent`, or `coomi-session`.
 *
