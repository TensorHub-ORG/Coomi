...
/**
 * Keyless built-artifact smoke: plain Node imports `@coomi/coomi-lsp` and
 * `@coomi/coomi-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@coomi/coomi-lsp')
      const LspLocal = await import('@coomi/coomi-lsp-stdio')
      const { default: LocalFileSystem } = await import('@coomi/coomi-fs-local')
