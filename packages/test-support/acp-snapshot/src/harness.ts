 ...
 *
 * @module @coomi/coomi-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `coomi-llm-replay` via `$COOMI_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/coomi-acp-snap-${key}`
}
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      COOMI_SNAPSHOT: opts.mode,
      COOMI_SNAPSHOT_FILE: opts.fixtureFile,
      COOMI_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      COOMI_SNAPSHOT_SPILL_ROOT: spillRoot,
      COOMI_HOME: join(cwd, '.coomi'),
      COOMI_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { COOMI_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { COOMI_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in coomi-llm-replay's loadSessionScripts
