 ...
 * @module @coomi/coomi-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@coomi/cordis'
import type {} from '@coomi/coomi-compaction'
import { decodeStorageRecord } from '@coomi/coomi-session'
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@coomi/coomi-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.COOMI_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $COOMI_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.COOMI_SNAPSHOT_OVERRIDE
  const childEnv = process.env.COOMI_SNAPSHOT_CHILD_FILES
