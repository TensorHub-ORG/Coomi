 ...
 * zero-build dev path, resolving `@coomi/coomi-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @coomi/coomi-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'COOMI_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.COOMI_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated coomi homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { COOMI_HOME: join(cwd, '.coomi'), COOMI_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
