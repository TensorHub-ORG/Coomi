import { createAssistantMessage, createUserMessage } from '@coomi/coomi-llm'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context, type Fiber } from '@coomi/cordis'
import { DatabaseSync } from 'node:sqlite'
import { chmod, mkdtemp, rm, stat, writeFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join } from 'node:path'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@coomi/coomi-session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@coomi/coomi-session'
import SessionPersistence, { SessionPersistenceRevision } from '@coomi/coomi-session-persistence'
import type { SessionPersistenceSnapshot } from '@coomi/coomi-session-persistence'
import SqliteSessionPersistence from '@coomi/coomi-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@coomi/coomi-session-query-sqlite'
import {
...
} from '@coomi/coomi-session-query'
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'coomi-session-search-'))
