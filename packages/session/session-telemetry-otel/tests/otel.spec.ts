...
import { getOrCreateAnonymousUserId } from '@coomi/coomi-anonymous-user-id'
import Loader from '@coomi/cordis-plugin-loader'
import { recordFeedback } from '@coomi/coomi-command-feedback'
...
// pin COOMI_HOME to a temp dir so the suite never touches the ambient ~/.coomi.
let tempHome: string
let previousCoomiHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'coomi-otel-home-'))
  previousCoomiHome = process.env.COOMI_HOME
  process.env.COOMI_HOME = tempHome
})
afterAll(() => {
  if (previousCoomiHome === undefined) delete process.env.COOMI_HOME
  else process.env.COOMI_HOME = previousCoomiHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'coomi-desktop' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@coomi/coomi-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@coomi/coomi-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@coomi/coomi-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('coomi-session-telemetry-otel real-load-path guard', () => {
