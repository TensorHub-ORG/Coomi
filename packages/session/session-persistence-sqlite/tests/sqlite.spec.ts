...
import type { Session, SessionEvent, SurfaceEvent, SurfaceEventType } from '@coomi/coomi-session'
import SqliteSessionPersistence, { SCHEMA_VERSION } from '@coomi/coomi-session-persistence-sqlite'
import {
  openDatabase,
  rowToEvent,
  rowToMeta,
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'coomi-sqlite-'))
  dirs.push(dir)
...
runCoordinatorContract('sqlite', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'coomi-sqlite-coord-'))
