...
import * as SessionStatsPlugin from '@coomi/coomi-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'coomi-session-stats-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@coomi/coomi-session-projection', SessionProjectionRegistry],
    ['@coomi/coomi-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@coomi/coomi-session-projection'",
      "- name: '@coomi/coomi-session-stats'",
    ])
