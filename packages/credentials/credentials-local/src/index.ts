/**
 * File-backed credentials provider over `$COOMI_HOME/.credentials.yaml`, layered
 ...
 * > $COOMI_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… coomi`, a CI
 ...
 * @module @coomi/coomi-credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@coomi/coomi-atomic-write'
import { canonicalizeWatchPath, resolveCoomiHome } from '@coomi/coomi-home-paths'
import { launchEnvironmentOf } from '@coomi/coomi-launch-environment'
import { CredentialProvider, credentialRef } from '@coomi/coomi-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@coomi/coomi-credentials'
import type { LaunchEnvironmentEntry } from '@coomi/coomi-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveCoomiHome(config.coomiHome), CREDENTIALS_FILENAME)),
...
/** File-backed credentials provider (`$COOMI_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start coomi from instead',
      )
    }
  }
