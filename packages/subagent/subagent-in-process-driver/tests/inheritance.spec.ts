...
import SandboxedFileSystem from '@coomi/coomi-fs-sandbox'
...
import { snapshotSubagentDescriptor } from '@coomi/coomi-subagent'
...
beforeEach(async () => {
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'coomi-inherit-')))
})
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current coomi file policy: read-only')
