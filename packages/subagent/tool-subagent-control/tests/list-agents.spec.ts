...
import type { SubagentListEntry } from '@coomi/coomi-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'coomi-tool-list-agents-'))
  roots.push(root)
...
describe('coomi-tool-subagent-control/list-agents', () => {
