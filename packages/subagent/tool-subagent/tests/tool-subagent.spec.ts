...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@coomi/coomi-tools'
import { assembleContextFor, type Agent } from '@coomi/coomi-agent'
...
import type { SubagentStartRequest } from '@coomi/coomi-subagent'
import LocalJobRegistry from '@coomi/coomi-jobs-local'
import * as SubagentSpawn from '@coomi/coomi-subagent-spawn-in-process'
...
/**
 * Drives the REAL plugin body: mounts `coomi-tool-subagent` on a real
...
describe('coomi-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @coomi/coomi-jobs')
  })
...
describe('coomi-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'coomi-tool-subagent-continuable-'))
    roots.push(root)
