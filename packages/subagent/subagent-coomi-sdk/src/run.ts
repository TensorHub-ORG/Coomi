/**
 * Fresh-process SDK subagent client. Drives one child Coomi
 * runtime over stdio JSON-RPC through `@coomi/coomi-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@coomi/coomi-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @coomi/coomi-subagent-coomi-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { DeepSeekHarness, type HarnessNotification } from '@coomi/coomi-sdk-client'
import type { ContentBlock } from '@coomi/coomi-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@coomi/coomi-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@coomi/coomi-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@coomi/coomi-subagent'
import { scrubbedParentEnv } from '@coomi/coomi-subprocess'

/** Resolved spawn spec for an SDK runtime child process (no defaults — see Config). */
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `coomi-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `COOMI_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `COOMI_*` fact survives while ambient namesakes never leak.
   */
