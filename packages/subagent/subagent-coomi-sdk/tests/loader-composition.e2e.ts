...
import { resolveExampleLaunch, runLoaderSmoke } from '@coomi/coomi-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-coomi-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'coomi-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'coomi-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        COOMI_TEST_CHILD_COMMAND: childLaunch.command,
        COOMI_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        COOMI_TEST_CHILD_ENV: JSON.stringify({
