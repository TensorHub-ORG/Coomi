 ...
 * @module @coomi/coomi-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@coomi/coomi-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `COOMI_*` entries are deployment-owned facts for the child harness
   * (e.g. `COOMI_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and COOMI_* names
