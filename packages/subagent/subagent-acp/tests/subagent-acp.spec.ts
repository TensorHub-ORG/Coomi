...
import type { SubprocessOutcome } from '@coomi/coomi-subprocess'
...
import { spawnSubprocess } from '@coomi/coomi-subprocess-local/src/spawn.ts'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit COOMI_* config entries to the child', async () => {
    // A deployment sets child-harness facts like COOMI_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'COOMI_ACP_TEST_FACT', COOMI_ACP_TEST_FACT: 'managed' })
...
describe('disposeAcpChild (the backend-owned teardown ladder over seam verbs)', () => {
  ...
  it('observes a spawn-level rejection and returns without a process to reap', async () => {
    const child = spawnSubprocess({
      ...
      cwd: '/nonexistent-dir-coomi-acp-ladder-test',
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1000 }, stderr: { maxBytes: 1000 } },
      graceMs: 200,
    })
...
describe('cwd resolution', () => {
  ...
  it('resolves a relative config cwd against the launch directory at load', async () => {
    ...
    // ABSOLUTE form — coomi's own ACP server rejects a relative session cwd, and
