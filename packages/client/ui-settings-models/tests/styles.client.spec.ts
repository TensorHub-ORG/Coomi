 ...
 *
 * The section paints in both themes, and a `--coomi-*` name the theme does not
...
describe('ModelsSection theme styles', () => {
  it('names only theme variables the token sheet defines', () => {
    // A `--coomi-*` name the sheet never declares is not a near miss: it silently
    ...
    // Every theme-variable prefix the sheets actually use, not just `--coomi-`:
    // a `--coomi-` name reads as a plausible sibling and would otherwise slip
    // past this gate into a fallback literal.
    const named = [...css.matchAll(/var\((--(?:dsw|coomi|ds)-[a-z0-9-]+)/g)].map(match => match[1])
  ...
  it('separates the row card from the editor it expands into', () => {
    ...
    expect(block('.editor')).toContain('background: var(--coomi-alias-bg-module-platform)')
    expect(block('.rowCard')).toContain('border: 1px solid var(--coomi-alias-border-l2)')
  ...
  it('never falls back to a literal colour', () => {
    ...
    expect(css).not.toMatch(/var\(--coomi-[a-z0-9-]+\s*,\s*(?:#|rgb|rgba|hsl|hsla)/)
  })
})
