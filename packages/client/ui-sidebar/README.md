...
Sidebar shell plugin: the wordmark, New Session action, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. In the Electron desktop shell, the wordmark and collapse control occupy the reserved native title-bar overlay above its bottom divider and remain interactive inside its draggable region; browser layouts keep the row in the sidebar. The collapsed desktop rail keeps only the Coomi mark as its click target and does not swap to another glyph or tooltip on hover. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
