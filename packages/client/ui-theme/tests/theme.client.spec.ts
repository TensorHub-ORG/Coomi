...
import { stubSettingsScope, type StubSettingsScope } from '@coomi/coomi-client-test-runtime'
import type {
  ThemeSettings,
  ThemeSnapshot,
  ThemeTokenOverrides,
} from '@coomi/coomi-client-ui-theme/client'
import { ThemeRuntime } from '@coomi/coomi-client-ui-theme/client'
...
describe('ThemeRuntime', () => {
  ...
  it('registered themes join the snapshot; disposing the active one resets to default', () => {
    const { theme, events, host } = make()
    const dispose = theme.register({ id: 'sepia', colorScheme: 'light', tokens: { '--coomi-alias-bg-base': 'red' } })
    expect(theme.getTheme().themes.map(t => t.id)).toEqual(['light', 'dark', 'sepia'])
    theme.setTheme('sepia')
    expect(theme.getTheme().active.tokens['--coomi-alias-bg-base']).toBe('red')
    dispose()
  ...
  it('exports sorted built-in, registered, and override-only token descriptions as copies', () => {
    ...
    theme.register({
      ...
      colorScheme: 'light',
      tokens: {
        '--coomi-alias-bg-base': 'duplicate-built-in',
    ...
    expect(tokens.filter(token => token.name === '--coomi-alias-bg-base')).toHaveLength(1)
