 ...
 * --coomi-alias-scrollbar-* token design-platform.css defines has a consumer,
...
const DARK_ATTRIBUTE = '[data-coomi-dark-theme]'
/** Alias tokens under test: the prefix the elevation pairs share. */
const TOKEN_PREFIX = '--coomi-alias-scrollbar-'
/** Prefix of the rebindable indirection scrollbar.css owns. */
const INDIRECTION_PREFIX = '--coomi-scrollbar-'
...
const ELEVATED_REBIND = new Map([
  ['--coomi-scrollbar-thumb', '--coomi-alias-scrollbar-bg-l2'],
  ['--coomi-scrollbar-thumb-hover', '--coomi-alias-scrollbar-hover-l2'],
 ...
 * @returns every `--coomi-*` token the sheet's rendering declarations depend on.
 */
function tokensRendered(rules: CssRule[]): Set<string> {
  ...
  while (pending.length > 0) {
    for (const name of varReferences(pending.pop()!)) {
      if (name.startsWith('--coomi-')) reached.add(name)
 ...
 * so something a scrollbar can sit against. `--coomi-alias-button-*`,
 * `--coomi-alias-interactive-*`, and `--coomi-alias-markdown-*` reach the same dark
 ...
 */
const SURFACE_TOKEN_PATTERN = /^--coomi-(?:alias-bg-|specific-)/
...
function elevatedRungs(): Set<string> {
  ...
  const rungs = new Set([resolve('--coomi-alias-bg-layer-2'), resolve('--coomi-alias-bg-layer-3')])
    ...
    for (const rule of platformRules) {
      for (const [property, value] of rule.declarations) {
        ...
        for (const reference of varReferences(value)) {
          expect(reference, `${property}: ${value}`).toMatch(/^--coomi-static-/)
        }
      }
    }
  })
...
describe('scrollbar.css width variable', () => {
  ...
  it('every reader of the width variable outside ui-theme references a defined variable', () => {
    ...
    // (`right: var(--coomi-scrollbar-width)`); a rename in scrollbar.css without
...
describe('elevated surface rebinds', () => {
  ...
  it('rebinds the pair to one target: the l2 elevation pair, or transparent', () => {
    ...
    // `thumb-hover: var(--coomi-alias-scrollbar-hover-l2)` — which repaints the
    ...
    // A shape check admits `color-mix(…, var(--coomi-alias-scrollbar-bg-l2) 85%,
  ...
  it('resolves the elevated surface set from the palette ladder', () => {
    ...
    // defined. `--coomi-specific-tip` is the regression that proved the point: it
    ...
    expect(elevatedSurfaces).toContain('--coomi-alias-bg-layer-2')
    expect(elevatedSurfaces).toContain('--coomi-alias-bg-layer-3')
    expect(elevatedSurfaces).toContain('--coomi-specific-menu')
    expect(elevatedSurfaces).toContain('--coomi-specific-input-major')
    expect(elevatedSurfaces).toContain('--coomi-specific-tip')
    // Base surfaces stay out, or every scroll container would be in scope and
    // the check would say nothing.
    expect(elevatedSurfaces).not.toContain('--coomi-alias-bg-base')
    expect(elevatedSurfaces).not.toContain('--coomi-alias-bg-layer-1')
  })
  ...
  it('every sheet that scrolls on an elevated surface rebinds', () => {
    ...
    // `--coomi-alias-bg-*` and `--coomi-specific-*` name a surface, so a floating
