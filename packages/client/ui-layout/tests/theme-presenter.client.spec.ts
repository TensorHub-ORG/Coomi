...
import type { ThemeSnapshot } from '@coomi/coomi-client-ui-theme/client'
import { DARK_ATTRIBUTE, ThemePresenter } from '@coomi/coomi-client-ui-layout/src/client/theme-presenter.ts'
...
describe('ThemePresenter', () => {
  ...
  it('applies tokens as inline variables and clears the previous set on theme change', () => {
    const presenter = new ThemePresenter()
    presenter.apply(snapshot('dark', { '--coomi-alias-bg': '#111', '--coomi-alias-fg': '#eee' }))
    expect(document.body.style.getPropertyValue('--coomi-alias-bg')).toBe('#111')
    expect(document.body.style.getPropertyValue('--coomi-alias-fg')).toBe('#eee')
    presenter.apply(snapshot('light', { '--coomi-alias-bg': '#fff' }))
    expect(document.body.style.getPropertyValue('--coomi-alias-bg')).toBe('#fff')
    // The old theme's extra variable is gone, not merged.
    expect(document.body.style.getPropertyValue('--coomi-alias-fg')).toBe('')
  })
  ...
  it('dispose removes color-scheme, the attribute, and every applied variable, sparing foreign inline styles', () => {
    ...
    const presenter = new ThemePresenter()
    presenter.apply(snapshot('dark', { '--coomi-alias-bg': '#111' }))
    const meta = themeColorMeta()
    presenter.dispose()
    expect(document.documentElement.style.colorScheme).toBe('')
    expect(document.body.hasAttribute(DARK_ATTRIBUTE)).toBe(false)
    expect(document.body.style.getPropertyValue('--coomi-alias-bg')).toBe('')
