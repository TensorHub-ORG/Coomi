...
afterEach(cleanup)
import { AppRoot } from '@coomi/coomi-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@coomi/coomi-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@coomi/coomi-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@coomi/coomi-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
