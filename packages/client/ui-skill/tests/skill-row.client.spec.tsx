...
import type { RunningToolCall, ToolResultNode } from '@coomi/coomi-client-runtime/client'
...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"coomi-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"coomi-manage-issues"}'): RunningToolCall {
  return {
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    expect(view.container.textContent).not.toContain('{"name":"coomi-manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('coomi-manage-issues')
