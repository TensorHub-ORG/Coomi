...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@coomi/coomi-llm/message'
import { CallId } from '@coomi/coomi-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@coomi/coomi-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@coomi/coomi-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@coomi/coomi-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@coomi/coomi-commands/types'
import { deriveEventMessage, foldSurface } from '@coomi/coomi-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@coomi/coomi-host-apiproxy/api'
 ...
 * `--coomi-*` tokens, a bold run, column-aligned table rows that must scroll
 ...
 * presenter's shape (see formatGrepOutput in coomi-tool-fs-search): a
 ...
 * formatGlobOutput in coomi-tool-fs-search).
 */
...
const WEB_SEARCH_RESULT: Omit<Extract<ToolResultView, { card: 'web'; kind: 'search' }>, 'card' | 'kind'> = {
  answer: 'Coomi is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://github.com/coomi/coomi-desktop',
      title: 'Coomi — plugin-based agent harness',
...
/** Fixture-local mirror of the goal projection value (coomi-goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@coomi/coomi-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@coomi/coomi-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@coomi/coomi-tool-read'\n" }],
  ])
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      ...
      'deepseek-web', 'coomi-desktop', 'deepseek-app', 'deepseek-landing-blog',
    ]],
  ])
