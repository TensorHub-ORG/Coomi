 ...
 * Everything else under @coomi/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@coomi\/coomi-(host-apiproxy|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@coomi\/coomi-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@coomi/coomi-client-runtime/client'
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.COOMI_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.COOMI_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.COOMI_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @coomi value import is a build error — a
      ...
      name: 'coomi-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@coomi/')) return null
