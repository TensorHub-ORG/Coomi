 ...
 * @module @coomi/coomi-client-modules/client
 */
import type { Context } from '@coomi/cordis'
import type { coomiWindow } from './manifest.ts'
...
export type {
  ...
  ClientModuleSystemOptions, ClientPluginHandoff, coomiWindow, WebBootEntry, WebBootGraph,
} from './manifest.ts'
...
export function apply(ctx: Context): void {
  const modules = (globalThis as coomiWindow).__COOMI_MODULES__
  ...
  if (modules === undefined) {
    throw new Error('client-modules: window.__COOMI_MODULES__ missing — the shell kernel must construct the module system before plugin boot')
  }
