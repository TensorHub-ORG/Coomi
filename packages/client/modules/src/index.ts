/**
 * Node half of the client module system (`coomi.client` dual-face package): scans
 * the host Loader's entries for packages declaring `coomi.client`, composes the
 * `window.__COOMI_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @coomi/coomi-client-modules
 */
...
import { Service } from '@coomi/cordis'
import type { Context } from '@coomi/cordis'
import type {} from '@coomi/cordis-plugin-loader'
...
/** package.json `coomi.client` declaration fields, validated one by one after reading the file. */
interface coomiClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `coomi.client` package (cached per name, never expires). */
interface PkgMeta {
  clientPath: string
  inject?: string[]
  immediately: boolean
...
/** Narrow an unknown parsed JSON value to the `coomi.client` declaration, throwing on malformed fields. */
function parseCoomiClient(pkgName: string, value: unknown): coomiClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object coomi.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} coomi.client.platform must be a string`)
  }
  if (decl.inject !== undefined && (!Array.isArray(decl.inject) || decl.inject.some(i => typeof i !== 'string'))) {
    throw new Error(`client-modules: ${pkgName} coomi.client.inject must be a string array`)
  }
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} coomi.client.immediately must be a boolean`)
  }
  return {
...
/**
 * Inject the boot entry graph into index.html: `window.__COOMI_BOOT__` as the
...
export function injectBootManifest(html: string, graph: WebBootGraph): string {
  ...
  const script = `<script>window.__COOMI_BOOT__ = ${json}</script>`
...
/**
 * The web plugin table service: incremental `coomi.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `coomi.client` declaration) are
   ...
   * @returns the graph served as `window.__COOMI_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const coomi = pkg.coomi
    const decl = parseCoomiClient(
      pkgName,
      coomi !== null && typeof coomi === 'object' ? (coomi as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares coomi.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
