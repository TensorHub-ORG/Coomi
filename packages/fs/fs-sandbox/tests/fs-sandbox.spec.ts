...
import { FsError, FsTargetKey } from '@coomi/coomi-fs'
import type { FsTarget } from '@coomi/coomi-fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.coomi-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'coomi-fssbx-tmp-')), 'temp.txt')
