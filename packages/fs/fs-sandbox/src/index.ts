 ...
 * `@coomi/coomi-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@coomi/coomi-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@coomi/coomi-tool-fs`),
 * exactly as bash's does.
 *
 * @module @coomi/coomi-fs-sandbox
 */

import { Context } from '@coomi/cordis'
import { LocalFileSystem } from '@coomi/coomi-fs-local'
import type { Config as LocalConfig } from '@coomi/coomi-fs-local'
import { FsError } from '@coomi/coomi-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@coomi/coomi-fs'
import { writableRoots } from '@coomi/coomi-sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@coomi/coomi-sandbox'
import type {} from '@coomi/coomi-sandbox-policy'
 ...
 * INSTEAD OF `coomi-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `coomi-tool-fs` resolves
