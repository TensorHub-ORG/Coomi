 ...
 * model-facing render consumes ({@link module:@coomi/coomi-tool-fs-search/search-core}
 ...
 * final output budget (`coomi-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @coomi/coomi-tool-fs-search/presentation
 */
 ...
 * ({@link module:@coomi/coomi-tool-fs-search/grep} `formatGrepMatches`), so
