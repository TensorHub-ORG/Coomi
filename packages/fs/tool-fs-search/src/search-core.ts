 ...
 *
 * @module @coomi/coomi-tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@coomi/cordis'
import { HarnessError } from '@coomi/coomi-llm'
import { ItemRetainer, TextRetainer } from '@coomi/coomi-output-retention'
import type { RetainedItems } from '@coomi/coomi-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@coomi/coomi-subprocess'
import type { SaveTextSpill, SpillRef } from '@coomi/coomi-spill'
 ...
 * `@coomi/coomi-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`coomi-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@coomi/coomi-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@coomi/coomi-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@coomi/coomi-tool-fs-search/presentation} `grepSearchMeta`)
