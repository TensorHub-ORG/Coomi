...
import ToolRuntime, { type ToolResult } from '@coomi/coomi-tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@coomi/coomi-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'coomi-tool-fs-session-cwd-'))
