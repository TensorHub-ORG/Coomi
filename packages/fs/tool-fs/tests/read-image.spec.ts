...
import { CodeRuntime } from '@coomi/coomi-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@coomi/coomi-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@coomi/coomi-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@coomi/coomi-llm'
import SystemPrompt from '@coomi/coomi-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@coomi/coomi-tools'
import type { Config as ToolConfig } from '@coomi/coomi-tools'
import LocalFileSystem from '@coomi/coomi-fs-local'
import * as FsPolicy from '@coomi/coomi-fs-observation-policy'
import LocalAttachmentStore from '@coomi/coomi-attachment-local'
import { AttachmentId, AttachmentStore } from '@coomi/coomi-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'coomi-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'coomi-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { coomiHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { coomiHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { coomiHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
