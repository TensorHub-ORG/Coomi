 ...
 * @module @coomi/coomi-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSendResult, TerminalSessionId } from '@coomi/coomi-terminal'
...
const SHELL_PROMPT = '__COOMI_PERSISTENT_BASH_PROMPT__ '
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__COOMI_PERSISTENT_BASH_START_${nonce}__`,
    end: `__COOMI_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __coomi_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__coomi_persistent_bash_status"`
}
