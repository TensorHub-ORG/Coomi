 ...
 * `@coomi/coomi-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `coomi-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `COOMI_*` environment through the
 ...
 * exit-status parse from `@coomi/coomi-shell`.
 *
 * @module @coomi/coomi-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@coomi/cordis'
import z from '@coomi/schemastery'
import { defineTool, TOOL_ABORTED } from '@coomi/coomi-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@coomi/coomi-tools'
import { HarnessError } from '@coomi/coomi-llm'
import type { Agent } from '@coomi/coomi-agent'
import type {} from '@coomi/coomi-system-prompt'
import type {} from '@coomi/coomi-jobs'
import type {} from '@coomi/coomi-shell-env'
import type {} from '@coomi/coomi-user-approval'
import type { SandboxExecutionPolicy, SandboxMode } from '@coomi/coomi-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@coomi/coomi-sandbox'
import type { SandboxPolicyService } from '@coomi/coomi-sandbox-policy'
import type { ShellRunResult } from '@coomi/coomi-shell'
import { parseExitStatus } from '@coomi/coomi-shell'
...
/* jscpd:ignore-start -- minimal mirror of coomi-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:COOMI_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror coomi-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of coomi-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of coomi-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of coomi-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors coomi-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with coomi-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors coomi-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        coomiEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @coomi/coomi-jobs and @coomi/coomi-tool-jobs')
        }
