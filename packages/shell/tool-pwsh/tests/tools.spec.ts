 ...
 * managed `COOMI_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@coomi/coomi-system-prompt'
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@coomi/coomi-shell'
...
import type { ShellProcessRead } from '@coomi/coomi-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, coomiHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, coomiHome === undefined ? {} : { coomiHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, coomiHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed COOMI_* environment', async () => {
    const coomiHome = mkdtempSync(join(tmpdir(), 'coomi-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, coomiHome)
    ...
    expect(request?.coomiEnv).toEqual({
      COOMI_HOME: coomiHome,
      COOMI_SHELL: '1',
      COOMI_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const coomiEnv = bash.requests[0]?.coomiEnv
    expect(coomiEnv).toBeDefined()
    expect(coomiEnv?.['COOMI_SHELL']).toBe('1')
    expect(coomiEnv?.['COOMI_HOME']).toEqual(expect.any(String))
    expect(coomiEnv).not.toHaveProperty('COOMI_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'coomi-tool-pwsh-policy-'))
