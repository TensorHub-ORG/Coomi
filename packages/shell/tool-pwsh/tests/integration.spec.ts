/**
 * Integration tests: the REAL `@coomi/coomi-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@coomi/coomi-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@coomi/coomi-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'coomi-tool-pwsh-'))
