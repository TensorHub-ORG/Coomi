...
The model-facing `pwsh` tool registered over the `ctx.shell` executor seam. Intended for Windows compositions where a PowerShell executor (e.g. `@coomi/coomi-pwsh-local`) backs `ctx.shell`; the tool contract is PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables. Behavior mirrors `coomi-tool-bash` call-for-call — foreground and `run_in_background` execution through the generic job runtime, the managed `COOMI_*` environment through the shared `shell-env` registry, the sandbox denial rendering with the same-turn `sandbox_permissions` escalation surface, and the bash marker/truncation rendering story (a clean exit produces no marker).
...
Every foreground and background model pwsh call receives a freshly collected trusted `COOMI_*` environment through the shared [`coomi-shell-env`](../shell-env/) registry: `COOMI_HOME` (the absolute Harness home), `COOMI_SHELL=1`, the agent's `COOMI_SESSION_ID`, and `COOMI_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `COOMI_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.coomiEnv` channel; `process.env` is never modified. The description teaches the generic `$env:COOMI_*` convention rather than naming persistence-specific variables.
...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#coomidsh-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
