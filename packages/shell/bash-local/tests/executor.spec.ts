...
const spillDir = mkdtempSync(join(tmpdir(), 'coomi-bash-exec-spec-'))
...
describe('LocalBashExecutor.run', () => {
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'true', workdir: '/nonexistent-coomi' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/coomiEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: 'cat; echo "[$SEAM_VAR][$COOMI_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
...
describe('LocalBashExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: 'cat; echo "[$BG_VAR][$COOMI_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      coomiEnv: { COOMI_BG_VAR: 'bg-coomi-env' },
    }))
    const output = await readUntil(proc, '[bg-env][bg-coomi-env]')
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'true', workdir: '/nonexistent-coomi' }))
