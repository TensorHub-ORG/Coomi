...
import { ShellExecutor } from '@coomi/coomi-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@coomi/coomi-shell'
import SystemPrompt from '@coomi/coomi-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@coomi/coomi-tools'
...
import ApprovalService from '@coomi/coomi-user-approval'
import type { ApprovalOutcome } from '@coomi/coomi-user-approval'
...
const spillDir = mkdtempSync(join(tmpdir(), 'coomi-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-coomi' })
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @coomi/coomi-jobs and @coomi/coomi-tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingCoomiHome = join(spillDir, 'coomi-home')
   ...
   * (the credential scrub in coomi-bash-local is the security control; see the
  ...
  class RecordingBashExecutor extends ShellExecutor {
    ...
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      return {
        ...
        ...request.coomiEnv !== undefined ? { coomiEnv: request.coomiEnv } : {},
        sandboxPolicy: request.sandboxPolicy,
      }
    }
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { coomiHome: recordingCoomiHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$COOMI_*')
    expect(description).not.toContain('COOMI_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.coomiEnv).toEqual({
      COOMI_HOME: recordingCoomiHome,
      COOMI_SESSION_ID: 'request-fg',
      COOMI_SESSION_JSONL: path,
      COOMI_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { COOMI_SESSION_ID: 'spoofed', COOMI_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.COOMI_SESSION_ID
    ...
    expect(bash.requests[0]?.coomiEnv).toEqual({
      COOMI_HOME: recordingCoomiHome,
      COOMI_SESSION_ID: 'request-id-only',
      COOMI_SHELL: '1',
    })
    expect(process.env.COOMI_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.coomiEnv)).toEqual([
      {
        COOMI_HOME: recordingCoomiHome,
        COOMI_SESSION_ID: 'request-parent',
        COOMI_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        COOMI_SHELL: '1',
      },
      {
        COOMI_HOME: recordingCoomiHome,
        COOMI_SESSION_ID: 'request-child',
        COOMI_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        COOMI_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.coomiEnv?.COOMI_SESSION_JSONL).not.toBe(bash.requests[1]?.coomiEnv?.COOMI_SESSION_JSONL)
  })
