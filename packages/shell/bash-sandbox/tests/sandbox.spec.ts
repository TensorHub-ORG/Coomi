...
import type { ShellRunResult, CollectedOutput } from '@coomi/coomi-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@coomi/coomi-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@coomi/coomi-sandbox'
import { SandboxPolicyService } from '@coomi/coomi-sandbox-policy'
import { SandboxBashExecutor } from '@coomi/coomi-bash-sandbox'
...
import type { Config } from '@coomi/coomi-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'coomi-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'COOMI_WRAP=1', 'bash', '-c', 'printf "%s" "$COOMI_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$COOMI_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'coomi-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.COOMI_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        coomiEnv: { COOMI_ORDER_FILE: order },
      }))
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'coomi-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'coomi-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
