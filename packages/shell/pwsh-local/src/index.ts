 ...
 *
 * @module @coomi/coomi-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors coomi-bash-local call-for-call by
...
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@coomi/coomi-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@coomi/coomi-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@coomi/coomi-subprocess'
import { installSettingsSection } from '@coomi/coomi-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@coomi/coomi-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of coomi-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `coomi-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.coomiEnv },
    }
  }
   ...
   * `coomi-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@coomi/coomi-pwsh-sandbox`.
