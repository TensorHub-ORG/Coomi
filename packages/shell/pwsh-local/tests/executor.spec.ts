/**
 * Real-process tests for `@coomi/coomi-pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@coomi/coomi-pwsh-local'
import LocalSubprocessRuntime from '@coomi/coomi-subprocess-local'
import SubprocessRuntime from '@coomi/coomi-subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@coomi/coomi-subprocess'
import { MAX_TIMER_DELAY_MS } from '@coomi/coomi-timeout'
import type { ShellProcess } from '@coomi/coomi-shell'

const spillDir = mkdtempSync(join(tmpdir(), 'coomi-pwsh-exec-spec-'))
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'coomi-pwsh-resolve-'))
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'coomi-pwsh-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'coomi-pwsh-resolve-dir-'))
    const store = join(dir, 'store')
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'coomi-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'coomi-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-coomi' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/coomiEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:COOMI_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      coomiEnv: { COOMI_SEAM_VAR: 'coomi-ok' },
    })
    ...
    expect(spec.coomiEnv).toEqual({ COOMI_SEAM_VAR: 'coomi-ok' })
    const result = await bash.run(spec)
    expect(lf(result.stdout.text)).toBe('piped\n[env-ok][coomi-ok]\n')
  })

  it('resolve() omits stdin/env/coomiEnv when the request supplies none', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({ command: 'Write-Output ok' })
    expect('stdin' in spec).toBe(false)
    expect('env' in spec).toBe(false)
    expect('coomiEnv' in spec).toBe(false)
  })
})
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:COOMI_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      coomiEnv: { COOMI_BG_VAR: 'bg-coomi-env' },
    }))
    const partialOutput = await readUntil(proc, '[bg-env][bg-coomi-env]')
    await proc.done
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-coomi' }))
