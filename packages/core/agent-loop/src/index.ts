 ...
 *
 * @module @coomi/coomi-agent-loop
 */

import { Context, FiberState, Service } from '@coomi/cordis'
import { randomUUID } from 'node:crypto'
import z from '@coomi/schemastery'
import { emitAgentEvent } from '@coomi/coomi-agent'
import type {
  Agent,
  AgentFactory,
  AgentHandle,
  ...
  ResumeAgentOptions,
  SessionStartSource,
} from '@coomi/coomi-agent'
import { errorChain } from '@coomi/coomi-llm'
import { installSettingsSection, settingsNamespace } from '@coomi/coomi-settings'
import { SessionId, SessionPreparation } from '@coomi/coomi-session'
import type { Session, SessionHeader } from '@coomi/coomi-session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a coomi-session-persistence backend)')
    }
