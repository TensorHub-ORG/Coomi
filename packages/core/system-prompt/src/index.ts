 ...
 *
 * @module @coomi/coomi-system-prompt
 */

import { Context, Service } from '@coomi/cordis'
import z from '@coomi/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@coomi/coomi-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@coomi/coomi-scope'
import type { ContextSnapshotSection, ToolSchema } from '@coomi/coomi-llm'
...
declare module '@coomi/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@coomi/coomi-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Coomi.',
      })
    }
    this.section({
