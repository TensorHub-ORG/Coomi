...
import type { Scope } from '@coomi/coomi-scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@coomi/coomi-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@coomi/coomi-tools'
import type { Agent } from '@coomi/coomi-agent'
import { Session, SessionId } from '@coomi/coomi-session'
import type { JsonValue, SessionEventMap } from '@coomi/coomi-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@coomi/coomi-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `coomi-agent-tool-presentation` produces.
