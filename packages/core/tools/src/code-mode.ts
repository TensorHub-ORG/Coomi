 ...
 * @module @coomi/coomi-tools/src/code-mode
 */

import { CallId, HarnessError } from '@coomi/coomi-llm'
import type { ContentBlock } from '@coomi/coomi-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@coomi/coomi-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`coomi-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
