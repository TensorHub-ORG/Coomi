 ...
 * execution pipeline.
 * @module @coomi/coomi-tools
 */

import { Context, Service } from '@coomi/cordis'
import z from '@coomi/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@coomi/coomi-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@coomi/coomi-scope'
import type { CallId, ContentBlock, ToolSchema } from '@coomi/coomi-llm'
import { assertNever, deepFreeze, HarnessError } from '@coomi/coomi-llm'
import type { Agent } from '@coomi/coomi-agent'
import { snapshotJsonValue } from '@coomi/coomi-session'
import type { JsonValue, UserMessage } from '@coomi/coomi-session'
import type { ToolProviderResult } from '@coomi/coomi-system-prompt'
import type { CodeRuntime } from '@coomi/coomi-code-runtime'
 ...
 * instead of deriving them: the seam's `coomi-code-runtime` README pair, its
...
export type { JsonValue } from '@coomi/coomi-session'
...
// lives in its own UI-facing module; re-export it so `@coomi/coomi-tools`
...
declare module '@coomi/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@coomi/coomi-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@coomi/coomi-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@coomi/coomi-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@coomi/coomi-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@coomi/coomi-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `coomi-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`coomi-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`coomi-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @coomi/coomi-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`coomi-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `coomi-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('coomi.tool.execution') as ToolExecutionToken
}
