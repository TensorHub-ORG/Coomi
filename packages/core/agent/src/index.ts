 ...
 *
 * @module @coomi/coomi-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@coomi/cordis'
import type { Fiber } from '@coomi/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@coomi/coomi-scope'
import type { Scoped } from '@coomi/coomi-scope'
import type { SessionEvent, SessionId } from '@coomi/coomi-session'
import type { TypertContext, TypertLookup } from '@coomi/coomi-typert-protocol'
...
declare module '@coomi/cordis' {
  interface Context {
     ...
     * `coomi-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in coomi-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `coomi-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `coomi-agent-loop` package.
 */
 ...
 * (`@coomi/coomi-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@coomi/coomi-agent#Agent',
        wireTypeSymbol: '@coomi/coomi-session/types#SessionId',
