...
import type { SubprocessHandle, SubprocessOutputReader } from '@coomi/coomi-subprocess'
...
const spillDir = mkdtempSync(join(tmpdir(), 'coomi-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-coomi-test' })).done)
...
describe('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@coomi/cordis')
    const { default: LocalSubprocessRuntime } = await import('@coomi/coomi-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-coomi-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-coomi-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient coomi env vars from child processes', async () => {
    process.env.COOMI_TEST_API_KEY = 'super-secret'
    process.env.COOMI_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.COOMI_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${COOMI_TEST_API_KEY:-absent}|${COOMI_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${COOMI_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.COOMI_TEST_API_KEY
      delete process.env.COOMI_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.COOMI_TEST_PLAIN
    }
  })

  it('forwards explicit COOMI_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient COOMI_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.COOMI_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${COOMI_STALE:-absent}|$COOMI_SHELL|$COOMI_SESSION_ID]"', {
        env: { COOMI_SHELL: '1', COOMI_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.COOMI_STALE
    }
  })
  ...
  it('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/coomi-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/coomi-subprocess-/)
