...
import { scrubbedParentEnv, SubprocessRuntime } from '@coomi/coomi-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and COOMI_ names (case-insensitively) but keeps PATH', () => {
    process.env.COOMI_SCRUB_PROBE = 'stale'
    process.env.coomi_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.COOMI_SCRUB_PROBE).toBeUndefined()
      expect(env.coomi_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.COOMI_SCRUB_PROBE
      delete process.env.coomi_scrub_probe_lower
