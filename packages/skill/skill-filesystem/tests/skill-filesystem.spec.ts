...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@coomi/coomi-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `coomi-${name}-`)))
}
...
describe('coomi-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.coomi/skills'), 'same', 'user coomi skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.coomi/skills'), 'same', 'project coomi skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.coomi/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project coomi skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-coomi')
    ...
    await writeSkill(join(noGit, '.coomi/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.coomi/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.coomi/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.coomi/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.coomi/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.coomi/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.coomi/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.coomi/skills/broken-link'))
    await symlink('/dev/null', join(home, '.coomi/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { coomiHome: join(home, '.coomi'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-coomi'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.coomi/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.coomi/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousCoomiHome = process.env.COOMI_HOME
    const previousAgentsHome = process.env.COOMI_AGENTS_HOME
    const previousBundledSkillDir = process.env.COOMI_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.COOMI_HOME = join(envHome, '.coomi')
      process.env.COOMI_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.COOMI_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.coomi/skills'), 'env-skill', 'Env skill')
      ...
      process.env.COOMI_HOME = join(envHome, 'empty-coomi')
      delete process.env.COOMI_BUNDLED_SKILL_DIR
      process.env.COOMI_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { coomiHome: join(envHome, 'empty-coomi') }).name).toBe('filesystem')
    } finally {
      if (previousCoomiHome === undefined) {
        delete process.env.COOMI_HOME
      } else {
        process.env.COOMI_HOME = previousCoomiHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.COOMI_AGENTS_HOME
      } else {
        process.env.COOMI_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.COOMI_BUNDLED_SKILL_DIR
      } else {
        process.env.COOMI_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
