...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const coomiHome = await mkdtemp(join(tmpdir(), 'coomi-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { coomiHome })
      ...
      await rm(coomiHome, { recursive: true, force: true })
    }
  })

  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const coomiHome = await mkdtemp(join(tmpdir(), 'coomi-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { coomiHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { coomiHome, maxImageBytes: 1 })
