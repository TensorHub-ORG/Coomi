 ...
 * `coomi --profile` launcher family.
 *
 * A profile is a directory under `$COOMI_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `coomi.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"coomi": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `coomi.profile.bundles` order over
 ...
 * first from the coomi installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$COOMI_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @coomi/coomi-app-boot/profile
 */
...
import type { EntryOptions } from '@coomi/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@coomi/cordis-plugin-include'
import { resolveCoomiHome } from '@coomi/coomi-home-paths'
...
/** The bundle half of the `coomi` manifest section: what a bundle package exports. */
export interface coomiBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `coomi` manifest section: what a profile directory composes. */
export interface coomiProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `coomi`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface coomiManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: coomiBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: coomiProfileManifest
}
...
export interface ProfileManifest {
  ...
  coomi?: coomiManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `coomi.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `coomi.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`coomi --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveCoomiHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveCoomiHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`coomi: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@coomi/coomi-base', '@coomi/coomi-web-app'],
  headless: ['@coomi/coomi-base', '@coomi/coomi-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@coomi/coomi-base', '@coomi/coomi-web-app', '@coomi/coomi-headless'],
}

/** The bundle list a `coomi plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@coomi/coomi-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this coomi profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `coomi.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `coomi-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      coomi: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`coomi: ${link} exists and is not a symlink; remove it so coomi can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$COOMI_HOME/profiles/node_modules`: one
 * symlink per package in the coomi app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`coomi-compaction`, `coomi-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the coomi app's package.json.
 * @param home - the Harness home; defaults to {@link resolveCoomiHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveCoomiHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (coomi-subprocess,
    // coomi-compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.coomi?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    coomi: {
      ...manifest.coomi,
      profile: { ...manifest.coomi?.profile, bundles: [...current] },
    },
  }
 ...
 * `@coomi/coomi-base` (and every other in-box bundle) always comes from
 * the same installation as the running coomi, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `coomi.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the coomi app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the coomi installation or ${profileDir}; `
    + `run 'coomi plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `coomi.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `coomi.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the coomi app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveCoomiHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveCoomiHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'coomi plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the coomi section entirely.
  const bundles = manifest.coomi?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.coomi?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no coomi.bundle in its package.json`)
    }
