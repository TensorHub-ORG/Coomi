/**
 * Shared boot glue for the app bins (`coomi`, `coomi-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.coomi`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @coomi/coomi-app-boot
 */
...
import { Context, type FiberState } from '@coomi/cordis'
import Loader, { type Entry, type EntryOptions } from '@coomi/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@coomi/cordis-plugin-include'
import Group from '@coomi/cordis-plugin-group'
import { coomiHomePath, resolveCoomiHome } from '@coomi/coomi-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@coomi/coomi-launch-environment'
import type {} from '@coomi/cordis-plugin-hmr'
...
declare module '@coomi/cordis' {
  interface Context {
    ...
    coomiHomePath?: typeof coomiHomePath
  }
}
...
export {
  ...
  type coomiBundleManifest,
  type coomiManifestSection,
  type coomiProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$COOMI_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['COOMI_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@coomi/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@coomi/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@coomi/cordis-plugin-group`
 ...
 * directory. The self-referential `coomi-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Coomi implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend coomi itself.`,
  })
}
