 ...
 *
 * @module @coomi/coomi-plan-mode
 */

import { Context, Service } from '@coomi/cordis'
import { z as zod } from 'zod'
import type { ZodType } from 'zod'
import type { Agent, PreStepDecision } from '@coomi/coomi-agent'
import { createUserMessage } from '@coomi/coomi-llm'
import type { Session, SessionEvent, UserMessage } from '@coomi/coomi-session'
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('coomi-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'coomi-plan-mode: close service lifetime')
