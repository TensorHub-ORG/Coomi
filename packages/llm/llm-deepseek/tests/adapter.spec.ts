...
import { createLaunchEnvironmentSnapshot } from '@coomi/coomi-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@coomi/coomi-anonymous-user-id'
import { SessionId } from '@coomi/coomi-session'
import * as LlmDeepSeek from '@coomi/coomi-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@coomi/coomi-llm-deepseek'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'coomi-llm-deepseek-'))
  vi.stubEnv('COOMI_HOME', testHome)
})
...
describe('DeepSeekAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-coomi-desktop-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-coomi-desktop-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-coomi-desktop-compact')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-coomi-desktop-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-coomi-desktop-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-coomi-desktop-compact']).toBe('1')
  })
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.coomi/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
