...
import LocalCredentialProvider from '@coomi/coomi-credentials-local'
import { settingsNamespace } from '@coomi/coomi-settings'
import FileSettingsProvider from '@coomi/coomi-settings-file'
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'coomi-llm-composition-'))
  vi.stubEnv('COOMI_HOME', root)
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@coomi/coomi-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@coomi/coomi-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@coomi/coomi-settings-file', FileSettingsProvider],
    ['@coomi/coomi-credentials-local', LocalCredentialProvider],
    ['@coomi/coomi-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-deepseek real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-coomi-desktop-user-id']).toBe(getOrCreateAnonymousUserId())
