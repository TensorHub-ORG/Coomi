...
import { resolveRetryPolicy, RetryPolicySchema } from '@coomi/coomi-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@coomi/coomi-llm'
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with coomi-llm-retry',
    )
  }
}
