 ...
 *
 * @module coomi-llm-pi-ai/replay
 */

import { LlmError } from '@coomi/coomi-llm'
import type { Message, ModelMessageSource } from '@coomi/coomi-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'coomi-foreign',
    provider: source?.provider ?? 'coomi-foreign',
    model: source?.model ?? 'coomi-foreign',
    usage: emptyPiUsage(),
