...
import LlmRuntime, { createUserMessage } from '@coomi/coomi-llm'
...
import { PiAiAdapter } from '@coomi/coomi-llm-pi-ai'
...
/** A throwaway $COOMI_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'coomi-pi-catalog-'))
  homes.push(dir)
