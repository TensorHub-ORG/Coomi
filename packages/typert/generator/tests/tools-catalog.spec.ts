...
import type { TypertContribution } from '@coomi/coomi-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@coomi/coomi-tool-cordis/src/api-catalog.ts'
...
describe('model-driven coomi-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@coomi/coomi-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('coomi-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@coomi/coomi-tools')
    ...
    const record = ctx.typert.getPackage('@coomi/coomi-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@coomi/coomi-tools', 'host')).toBeUndefined()
  })
})
