...
} from '@coomi/coomi-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@coomi/coomi-typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@coomi/coomi-tools', 'ToolInput')).toBe('@coomi/coomi-tools#ToolInput')
    expect(typertPackageKey('@coomi/coomi-tools', 'host')).toBe('@coomi/coomi-tools#host')
    expect(ctx.typert.get('@coomi/coomi-tools#ToolInput')).toMatchObject({
      package: '@coomi/coomi-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@coomi/coomi-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@coomi/coomi-tools', 'host')).toMatchObject({
      key: '@coomi/coomi-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@coomi/coomi-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@coomi/coomi-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@coomi/coomi-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@coomi/coomi-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@coomi/coomi-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@coomi/coomi-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@coomi/coomi-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@coomi/coomi-tools#ToolInput')).not.toBe(projected)
  })
