 ...
 * process-local registry lives in `@coomi/coomi-jobs-local`.
 * @module @coomi/coomi-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@coomi/coomi-jobs is the abstract job registry seam; load an implementation such as @coomi/coomi-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
