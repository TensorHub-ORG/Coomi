 ...
 * @module @coomi/coomi-jobs-local
 */

import { Context } from '@coomi/cordis'
import z from '@coomi/schemastery'
import type { Agent } from '@coomi/coomi-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@coomi/coomi-scope'
import type { ScopeLayer } from '@coomi/coomi-scope'
import { deadline, timeoutOf } from '@coomi/coomi-timeout'
import { JobRegistry, JobId } from '@coomi/coomi-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@coomi/coomi-jobs'
 ...
 * `@coomi/coomi-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @coomi/coomi-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @coomi/coomi-agent)')
    }
