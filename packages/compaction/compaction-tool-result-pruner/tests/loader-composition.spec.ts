...
describe('compaction-tool-result-pruner real Loader composition', () => {
  it('loads and resolves the flat YAML plugin shape', async () => {
    root = await mkdtemp(join(tmpdir(), 'coomi-compact-tool-result-prune-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@coomi/coomi-token-meter') return TokenMeter
        if (specifier === '@coomi/coomi-compaction-tool-result-pruner') return ToolResultPruner
