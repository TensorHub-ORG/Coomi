...
import { BasicCompactionEngine } from '@coomi/coomi-compaction-basic'
import SessionStore from '@coomi/coomi-session'
import TokenMeter from '@coomi/coomi-token-meter'
...
Loading the plugin registers `ctx.compaction`. Add [`coomi-compaction-tool-result-pruner`](../compaction-tool-result-pruner/README.md) as a sibling before this plugin to enable the optional model-free pass. With `auto: true` (the default) it compacts automatically under token pressure. The sibling [`coomi-command-compact`](../command-compact/README.md) calls `ctx.compaction.compactNow(...)`; programmatic callers may also use any seam operation directly.

For example, the same compact plugin can safely serve models with different capacities and one target-specific policy:

```yaml
- name: '@coomi/coomi-compaction-basic'
  config:
    thresholdRatio: 0.8
    retainRatio: 0.16
    modelPolicies:
