...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'coomi-token-meter-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ['@coomi/coomi-llm', LlmRuntime],
    ['@coomi/coomi-session', SessionStore],
    ['@coomi/coomi-token-meter', TokenMeter],
    ['@coomi/coomi-compaction-tool-result-pruner', ToolResultPruner],
    ['@coomi/coomi-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      "- name: '@coomi/coomi-llm'",
      "- name: '@coomi/coomi-session'",
      "- name: '@coomi/coomi-token-meter'",
      "- name: '@coomi/coomi-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
