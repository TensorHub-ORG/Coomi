...
import { writeFileAtomic } from '@coomi/coomi-atomic-write'
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'coomi-settings-local-'))
...
describe('boot and reads', () => {
  ...
  it('defaults the file location under the configured harness home', async () => {
    const dir = await tempDir()
    const ctx = await boot({ coomiHome: dir, watch: false })
