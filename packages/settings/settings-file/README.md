...
| `coomiHome` | Harness home used when `path` is omitted | `$COOMI_HOME` or `~/.coomi` |
...
- **Host configuration adapters receive the resolved path.** `ctx.settings.documentPath` is the absolute `resolveSpec()` filename, including a custom YAML/JSON path; `prepareDocument()` preserves an existing file or exclusively creates an absent empty file with owner-only permissions before the Host opens it. The browser receives only an availability flag, never reconstructs `$COOMI_HOME`, and never submits a filesystem target.
