...
import { createUserMessage, errorChain } from '@coomi/coomi-llm'
import {
  AgentSideConnection,
  ndJsonStream,
  PROTOCOL_VERSION,
...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  // coomi-subagent-acp. The bridge offers one-shot choices only and never infers a
    ...
    return {
      initialize(_params: InitializeRequest): Promise<InitializeResponse> {
        ...
        return Promise.resolve({
          ...
          agentInfo: { name: 'coomi-desktop-acp', version: '0.0.1' },
          agentCapabilities: {
