...
`resolveCoomiHome()` resolves the single-root Coomi home. Precedence, highest first: an explicit configured path, `$COOMI_HOME`, then `~/.coomi`. The harness keeps all user data under one root.

`coomiHomePath(...segments)` joins child segments onto that resolved home with Node's platform path rules. With no segments it returns the home itself.

`coomiHomeDisplay()` names an active root symbolically for user-facing paths: `~/.coomi` for the default home, `$COOMI_HOME` for any configured home. It never leaks an absolute machine path.

`COOMI_HOME_DIR_NAME` owns the default user-data directory name: `.coomi`.

`defaultCoomiHome()` returns the default Coomi home by joining the operating-system home directory with `.coomi`, using Node's platform path rules.
