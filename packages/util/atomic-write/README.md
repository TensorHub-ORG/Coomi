...
Zero-dependency atomic file replacement shared by file-backed stores that must never leave partial, symlink-hijacked, or wider-than-intended content on disk — the user-settings document (`coomi-settings-file`) and the credentials store (`coomi-credentials-local`).
...
await writeFileAtomic('/home/u/.coomi/settings.yaml', text, { mode: 0o600 })

// Read-modify-write against the same file from several processes.
await withFileLock('/home/u/.coomi/settings.yaml', async () => {
  await writeFileAtomic('/home/u/.coomi/settings.yaml', render(text), { mode: 0o600 })
})
```
