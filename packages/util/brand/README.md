...
A package brands the ids it owns — `CallId` in `coomi-llm`, the shared agent/session `SessionId` in `coomi-session`, and `JobId` in `coomi-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `coomi-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
