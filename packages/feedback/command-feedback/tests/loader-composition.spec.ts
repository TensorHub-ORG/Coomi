...
import type { Agent, AgentStatus } from '@coomi/coomi-agent'
import CommandRuntime from '@coomi/coomi-commands'
import SessionStore, { SessionId } from '@coomi/coomi-session'
import * as CommandFeedback from '@coomi/coomi-command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'coomi-command-feedback-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@coomi/coomi-commands', CommandRuntime],
      ['@coomi/coomi-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { COOMI_HOME: root } })
