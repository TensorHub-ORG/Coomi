...
import * as StorageDomain from '@coomi/coomi-storage-domain'
import * as StorageJson from '@coomi/coomi-storage-json'
import { remoteMethods } from '@coomi/coomi-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@coomi/coomi-session', SessionStore],
    ['@coomi/coomi-session-persistence-jsonl', JsonlSessionPersistence],
    ['@coomi/coomi-storage', Storage],
    ['@coomi/coomi-storage-json', StorageJson],
    ['@coomi/coomi-storage-domain', StorageDomain],
    ['@coomi/coomi-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'coomi-message-feedback-loader-'))
    const configPath = join(root, 'cordis.yml')
    await writeFile(configPath, [
      "- name: '@coomi/coomi-session'",
      "- name: '@coomi/coomi-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@coomi/coomi-storage'",
      "- name: '@coomi/coomi-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@coomi/coomi-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@coomi/coomi-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
