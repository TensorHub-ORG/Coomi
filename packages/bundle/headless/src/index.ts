/**
 * @coomi/coomi-headless — one-shot direct Agent driver. The bundle patch
 * rides over coomi-base without Host, HTTP, or browser plugins; this runner
...
import type { ModelSelectionRef } from '@coomi/coomi-agent'
import type {} from '@coomi/coomi-agent-default-model'
...
import type {} from '@coomi/coomi-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`coomi: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  // (@coomi/coomi-agent-presets README, "Composing a child agent").
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`coomi: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
