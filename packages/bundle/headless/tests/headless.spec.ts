...
import AgentDefaultModelConfig from '@coomi/coomi-agent-default-model'
import { createAssistantMessage } from '@coomi/coomi-llm'
import SessionStore from '@coomi/coomi-session'
import type { Session, UserMessage } from '@coomi/coomi-session'
...
describe('headless runner', () => {
  ...
  it('prints the durable model failure when the final turn ends in error', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'coomi: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    expect(err).toBe('coomi: factory exploded\n')
