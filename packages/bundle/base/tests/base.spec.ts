/**
 * The bundle's substance is its patch file: the `coomi.bundle.patch` manifest
...
describe('coomi-base bundle', () => {
  it('declares a parseable patch list through the coomi.bundle.patch manifest field', () => {
    const root = fileURLToPath(new URL('..', import.meta.url))
    const manifest = JSON.parse(
      readFileSync(resolve(root, 'package.json'), 'utf8'),
    ) as {
      dependencies?: Record<string, string>
      coomi?: { bundle?: { patch?: string } }
    }
    expect(manifest.coomi?.bundle?.patch).toBe('./cordis.patch.yml')
    const parsed = yaml.load(
      readFileSync(resolve(root, manifest.coomi!.bundle!.patch!), 'utf8'),
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.COOMI_TELEMETRY_MODE || 'DISABLED'",
    })
    expect(rows.filter(row => row.id === 'subagent-codex')).toHaveLength(0)
    expect(rows.filter(row => row.id === 'subagent-claude-code')).toHaveLength(0)
    expect(manifest.dependencies).not.toHaveProperty('@coomi/coomi-subagent-codex')
    expect(manifest.dependencies).not.toHaveProperty('@coomi/coomi-subagent-claude-code')
  })
