...
import { Inbox } from '@coomi/coomi-agent'
import type { Agent } from '@coomi/coomi-agent'
import { runLoaderSmoke } from '@coomi/coomi-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@coomi/coomi-e2b'
import TerminalSessionService, { TerminalSessionId } from '@coomi/coomi-terminal'
import { BashTerminalBackend } from '@coomi/coomi-terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', COOMI_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/coomi-e2b-bootstrap-profile-leak'
      ...
      ctx.provide('e2b', {
        cwd: '/home/user',
        runtimeRoot: '/home/user/.coomi-e2b',
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'coomi_leak=0',
          'for coomi_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$coomi_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$coomi_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then coomi_leak=1; fi',
          'done',
          'printf "DIRECT=<%s> LEAK=<%s>\\n" "${NPM_TOKEN-}" "$coomi_leak"',
        ].join('\n')],
        cwd: '/home/user',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> coomi=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$COOMI_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> coomi=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'coomi-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.coomi-e2b/terminals/')
