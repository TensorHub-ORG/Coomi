...
import type { CommandHandle, CommandResult, Sandbox } from '@coomi/coomi-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$coomi_e2b_env_bin" -i "$coomi_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$coomi_e2b_tee" --output-error=warn-nopipe >("$coomi_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$coomi_e2b_tee" --output-error=warn-nopipe >("$coomi_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'coomi_e2b_rm=$7',
    'shift 7',
    'coomi_e2b_pgid="$("$coomi_e2b_ps" -o pgid= -p "$$" | "$coomi_e2b_tr" -d " ")"',
    `printf '%s\\n' "$coomi_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t coomi_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$coomi_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$coomi_e2b_env_bin" -i -- "\${coomi_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'coomi_e2b_status=$?',
    `printf '%s\\n' "$coomi_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$coomi_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t coomi_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'coomi_e2b_env_bin="$(command -v env)"',
    'coomi_e2b_setsid="$(command -v setsid)"',
    'coomi_e2b_bash="$(command -v bash)"',
    'coomi_e2b_node="$(command -v node)"',
    'coomi_e2b_ps="$(command -v ps)"',
    'coomi_e2b_tr="$(command -v tr)"',
    'coomi_e2b_tee="$(command -v tee)"',
    'coomi_e2b_head="$(command -v head)"',
    'coomi_e2b_rm="$(command -v rm)"',
    'for coomi_e2b_tool in "$coomi_e2b_env_bin" "$coomi_e2b_setsid" "$coomi_e2b_bash" "$coomi_e2b_node" "$coomi_e2b_ps" "$coomi_e2b_tr" "$coomi_e2b_tee" "$coomi_e2b_head" "$coomi_e2b_rm"; do',
    '  [[ "$coomi_e2b_tool" == /* && -x "$coomi_e2b_tool" ]] || exit 125',
    'done',
    `exec "$coomi_e2b_env_bin" -i -- "\${coomi_e2b_env[@]}" "$coomi_e2b_setsid" --wait -- "$coomi_e2b_bash" -c ${quoteE2BShellArg(inner)} coomi-e2b "$coomi_e2b_env_bin" "$coomi_e2b_node" "$coomi_e2b_ps" "$coomi_e2b_tr" "$coomi_e2b_tee" "$coomi_e2b_head" "$coomi_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
