...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'coomi_state=$1',
  'mapfile -d \'\' -t coomi_env < "$coomi_state/environment"',
  'mapfile -d \'\' -t coomi_argv < "$coomi_state/argv"',
  'coomi_output_marker=$(<"$coomi_state/output-marker")',
  'rm -f -- "$coomi_state/environment" "$coomi_state/argv" "$coomi_state/output-marker" "$coomi_state/runner.bash"',
  'if (( ${#coomi_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$coomi_output_marker"',
  'exec env -i -- "${coomi_env[@]}" "${coomi_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`coomi-e2b-bootstrap:${randomUUID()}`)
