...
import { e2bControlEnvs } from '@coomi/coomi-e2b'
import type { Sandbox } from '@coomi/coomi-e2b'
import { SENSITIVE_ENV_PATTERN } from '@coomi/coomi-subprocess'
...
export async function readRemoteEnvironment(sandbox: Sandbox, signal?: AbortSignal): Promise<string> {
  ...
  const result = await sandbox.commands.run(
    'set -o pipefail; coomi_e2b_passwd="$(getent passwd "$(id -u)")"; IFS=: read -r _ _ _ _ _ coomi_e2b_home _ <<<"$coomi_e2b_passwd"; test -n "$coomi_e2b_home" -a -d "$coomi_e2b_home"; printf \'%s\' "$coomi_e2b_home" | base64 -w 0; printf \'\\n\'; env -0 | base64 -w 0',
...
export function scrubRemoteEnvironment(raw: string): Map<string, string> {
  ...
  for (const [name, value] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('COOMI_') || SENSITIVE_ENV_PATTERN.test(name)) continue
...
export function bootstrapEnvironment(raw: string): Record<string, string> {
  ...
  for (const [name] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('COOMI_') || SENSITIVE_ENV_PATTERN.test(name)) environment[name] = ''
  }
  return environment
}
