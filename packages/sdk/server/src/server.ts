 ...
 *
 * @module @coomi/coomi-sdk-jsonrpc-server/server
 */

import type { Context } from '@coomi/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@coomi/coomi-agent'
import { createUserMessage } from '@coomi/coomi-llm'
import { carrierKeyOf, type Scoped } from '@coomi/coomi-scope'
import { SessionId } from '@coomi/coomi-session'
import type SubagentRuntime from '@coomi/coomi-subagent'
import type { SubagentRunEndInfo } from '@coomi/coomi-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    return { serverInfo: { name: 'coomi-desktop-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown Coomi SDK runtime method: ${method}`)
    }
  }
