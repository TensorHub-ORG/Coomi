...
import type { SessionHeader, SessionId } from '@coomi/coomi-session'
import type { SessionLineageNode } from '@coomi/coomi-session-query'
import type { SessionRawArtifact } from '@coomi/coomi-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@coomi/coomi-host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('coomi-session-session-root.zip')
