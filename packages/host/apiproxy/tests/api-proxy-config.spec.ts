...
import LlmRuntime, { LlmAdapter } from '@coomi/coomi-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@coomi/coomi-llm'
import { SettingsProvider, settingsNamespace } from '@coomi/coomi-settings'
import type { SettingsNamespace } from '@coomi/coomi-settings'
import { CredentialProvider } from '@coomi/coomi-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@coomi/coomi-credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@coomi/coomi-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('coomi-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('coomi-credentials-local')
  })
