...
import { TypertLookupFailure } from '@coomi/coomi-typert-protocol'
import TypertRegistry from '@coomi/coomi-typert-registry'
import { createUserMessage, MessageId } from '@coomi/coomi-llm'
import type { Agent } from '@coomi/coomi-agent'
import UserQuestionService from '@coomi/coomi-user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@coomi/coomi-session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@coomi/coomi-session-persistence'
import type { RpcRequest } from '@coomi/coomi-host-apiproxy/api/rpc'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'coomi-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'coomi-cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@coomi/coomi-session').Session
