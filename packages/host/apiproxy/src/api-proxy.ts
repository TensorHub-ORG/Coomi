...
import { installModelSelection } from '@coomi/coomi-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@coomi/coomi-agent'
import type {} from '@coomi/coomi-agent-presets/types'
import { AttachmentError } from '@coomi/coomi-attachment'
import type { ImageAttachmentRef } from '@coomi/coomi-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@coomi/coomi-llm'
import { errorChain } from '@coomi/coomi-llm'
import type { ContentBlock, MessageSource } from '@coomi/coomi-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@coomi/coomi-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@coomi/coomi-session'
import type { SessionPersistence } from '@coomi/coomi-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@coomi/coomi-session-query'
import { SubagentError } from '@coomi/coomi-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@coomi/coomi-subagent'
import { isUserInvocable } from '@coomi/coomi-skill'
import type { Workspace, WorkspaceRecord } from '@coomi/coomi-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@coomi/coomi-workspace'
...
} from '@coomi/coomi-agent-presets'
import type { PresetBearingSession } from '@coomi/coomi-agent-presets'
...
import type { SessionRawArtifact } from '@coomi/coomi-session-persistence'
import {
...
import type {} from '@coomi/coomi-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@coomi/coomi-jobs'
import type { JobSnapshot } from '@coomi/coomi-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@coomi/coomi-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@coomi/coomi-goal'
import type { GoalRef as CoreGoalRef } from '@coomi/coomi-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@coomi/coomi-commands'
...
import type {} from '@coomi/coomi-cordis-host-runner/types'
import type {} from '@coomi/coomi-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@coomi/coomi-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@coomi/coomi-settings'
import { credentialRef } from '@coomi/coomi-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@coomi/coomi-session-title'
import type { CallId } from '@coomi/coomi-llm/brand'
import type { ScopeKey } from '@coomi/coomi-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@coomi/coomi-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@coomi/coomi-user-approval'
...
} from '@coomi/coomi-user-questions'
import { UserQuestionError } from '@coomi/coomi-user-questions'
import { DirectoryPickerError } from '@coomi/coomi-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @coomi/coomi-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: coomi-llm depends on coomi-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @coomi/coomi-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @coomi/coomi-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @coomi/coomi-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @coomi/coomi-session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in coomi-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts coomi-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @coomi/coomi-skill', details: {} })
        }
