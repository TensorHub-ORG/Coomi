...
import type { AttachmentStore, ImageAttachmentRef } from '@coomi/coomi-attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@coomi/coomi-session-query'
import type { SessionId, SessionStore } from '@coomi/coomi-session'
import type { SessionPersistence, SessionRawArtifact } from '@coomi/coomi-session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `coomi-session-${safeSessionIdSegment(sessionId)}.zip`
}
