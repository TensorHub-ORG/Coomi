...
import HttpServer from '@coomi/coomi-host-webserver'
import type { DirectoryPicker } from '@coomi/coomi-host-directory-picker'
import BrowseDirectoryPicker from '@coomi/coomi-host-directory-picker-browse'
import NativeDirectoryPicker from '@coomi/coomi-host-directory-picker-native'
...
const AUTO = '@coomi/coomi-host-directory-picker-auto'
const NATIVE = '@coomi/coomi-host-directory-picker-native'
const BROWSE = '@coomi/coomi-host-directory-picker-browse'
const NATIVE_SURFACE = '@coomi/coomi-client-ui-directory-picker-native'
const BROWSE_SURFACE = '@coomi/coomi-client-ui-directory-picker-browse'
...
async function loadComposition(
  ...
  root = await mkdtemp(join(tmpdir(), 'coomi-directory-picker-auto-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@coomi/coomi-host-webserver'",
    '  config:',
    `    host: '${bindHost}'`,
    '    port: 0',
    `- name: '${AUTO}'`,
  ...
  const modules = new Map<string, unknown>([
    ['@coomi/coomi-host-webserver', HttpServer],
...
function stubAttendedHost(): void {
  fakeBin = mkdtempSync(join(tmpdir(), 'coomi-picker-bin-'))
