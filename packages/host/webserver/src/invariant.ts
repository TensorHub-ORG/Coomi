/**
 * Package-owned invariant companion for `@coomi/coomi-host-webserver`.
 * @module @coomi/coomi-host-webserver/invariant
 */
...
const PACKAGE_NAME = '@coomi/coomi-host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__coomi_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__coomi_invariant_upgrade_probe__', handler: () => {} }
