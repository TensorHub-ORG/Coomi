...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@coomi/coomi-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@coomi/coomi-hook-protocol'
import type { RunHookOptions } from '@coomi/coomi-hook-protocol'
 ...
 * executor (coomi-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what coomi-bash-local's resolve does for the fields runHook sets.
      return {
