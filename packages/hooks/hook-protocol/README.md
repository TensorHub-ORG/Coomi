# @coomi/coomi-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@coomi/coomi-hooks-claude-code`, `@coomi/coomi-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`coomi-hook-protocol`) | The bridge (`coomi-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `coomi-hooks-claude-code` and `coomi-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
