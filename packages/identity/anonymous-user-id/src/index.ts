 ...
 * harness home resolved by {@link resolveCoomiHome} (`$COOMI_HOME` > `~/.coomi`),
 ...
 * machine: every process sharing one `$COOMI_HOME` reports the same id, and
 ...
 *
 * @module @coomi/coomi-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `COOMI_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveCoomiHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
