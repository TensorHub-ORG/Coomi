...
import { CompactionId, compactCheckpointSource } from '@coomi/coomi-compaction'
import { createUserMessage, CallId , createMessage, createToolResultMessage } from '@coomi/coomi-llm'
import SessionStore, { Session, SessionId } from '@coomi/coomi-session'
import SessionQueryEngine from '@coomi/coomi-session-query'
...
} from '@coomi/coomi-session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a coomi-session: URI?')).toEqual({
      text: 'what is a coomi-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see coomi-session:%%%')).toEqual({
      text: 'see coomi-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see coomi-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](coomi-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `coomi-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('coomi-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
