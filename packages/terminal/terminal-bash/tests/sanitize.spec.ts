...
import { normalizeTerminalText, TerminalSanitizer } from '@coomi/coomi-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07dsh> ')).toEqual({ text: 'coomi> ', prompt: true, promptTail: 'coomi> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('coomi> ')).toEqual({ text: 'coomi> ', prompt: false, promptTail: 'coomi> ' })
  })
