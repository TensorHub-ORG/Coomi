...
import AgentRegistry, { Inbox, type Agent } from '@coomi/coomi-agent'
import SandboxProvider from '@coomi/coomi-sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@coomi/coomi-sandbox'
import SandboxPolicyService, { setSandboxMode } from '@coomi/coomi-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@coomi/coomi-terminal'
import { BashTerminalBackend } from '@coomi/coomi-terminal-bash'
import * as ptyLocal from '@coomi/coomi-terminal-bash'
import type { ResolvedConfig } from '@coomi/coomi-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@coomi/coomi-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'coomi> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        COOMI_SHELL: '1', COOMI_SESSION_ID: 'agent', COOMI_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    expect(session.motd).toBe('coomi> ')
