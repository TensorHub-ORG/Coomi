...
The six generated schemas are listed in the [`coomi-tool-terminal` catalog section](../../../docs/tool-catalog.md#coomidsh-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@coomi/coomi-jobs` and its model-facing controller.
