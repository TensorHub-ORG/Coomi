...
export class WebRuntime extends Service {
   ...
   * `$COOMI_WEB_SEARCH_PROVIDER` / `$COOMI_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.COOMI_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.COOMI_WEB_FETCH_PROVIDER
  }
