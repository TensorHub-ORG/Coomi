...
import type { WebSearchProvider, WebSearchResult } from '@coomi/coomi-web'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@coomi/coomi-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by coomi-tool-call-timeout-policy over exec.signal.
