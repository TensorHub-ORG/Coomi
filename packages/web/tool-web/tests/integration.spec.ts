/**
 * Integration: the real fetch backend (`coomi-web-fetch-http`) + a real search provider
 * (`coomi-web-search-exa`) + the real seam (`coomi-web`) + the model tool (`coomi-tool-web`) + the
 * tool-call timeout policy (`coomi-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@coomi/coomi-tools'
...
import * as TimeoutPolicy from '@coomi/coomi-tool-call-timeout-policy'
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by coomi-tool-call-timeout-policy,
