...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@coomi/coomi-tools'
import type { WebSearchResult, WebSearchSource } from '@coomi/coomi-web'
import type {} from '@coomi/coomi-system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `coomi-tool-fs`'s
