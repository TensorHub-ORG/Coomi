 ...
 * remain session-free.
 *
 * @module @coomi/coomi-sandbox-policy
 */
...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@coomi/coomi-sandbox'
import type { Session } from '@coomi/coomi-session'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current coomi file policy: read-only. Any available operation enforced by the coomi file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current coomi file policy: workspace-write. Any available operation enforced by the coomi file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current coomi file policy: danger-full-access. The coomi file sandbox does not restrict file modifications by available operations.'
