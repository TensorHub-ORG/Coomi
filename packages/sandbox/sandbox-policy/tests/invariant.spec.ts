...
import SessionStore, { type Session, type SessionEvent } from '@coomi/coomi-session'
import InvariantRegistry, { InvariantError } from '@coomi/coomi-invariants'
import * as SandboxPolicyInvariant from '@coomi/coomi-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@coomi/coomi-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@coomi/coomi-sandbox-policy',
    })
  })
})
