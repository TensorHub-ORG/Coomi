 ...
 * @module @coomi/coomi-sandbox-local
 */
...
} from '@coomi/node-addon-landlock-run'
import { Context } from '@coomi/cordis'
import z from '@coomi/schemastery'
import { assertNever } from '@coomi/coomi-llm'
import { SandboxProvider, SandboxUnavailableError } from '@coomi/coomi-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@coomi/coomi-sandbox'
import type { SessionId } from '@coomi/coomi-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@coomi/coomi-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@coomi/coomi-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@coomi/coomi-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@coomi/coomi-sandbox-windows-acl/src/runner.ts'))
