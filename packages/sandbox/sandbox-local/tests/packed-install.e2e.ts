...
const platformPackageName = `@coomi/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @coomi peer (koffi arrives
  // from the registry).
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'coomi-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'coomi-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'coomi-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@coomi/node-addon-landlock-run'
