 ...
 * @module @coomi/coomi-agent-spine-demo
 */

import type { Context } from '@coomi/cordis'
import Timer from '@coomi/cordis-plugin-timer'
import z from '@coomi/schemastery'
import LlmRuntime from '@coomi/coomi-llm'
import SessionStore from '@coomi/coomi-session'
import SessionTitleService, { type Config as SessionTitleConfig } from '@coomi/coomi-session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@coomi/coomi-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@coomi/coomi-tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@coomi/coomi-skill'
import * as SkillFileSystem from '@coomi/coomi-skill-filesystem'
import AgentRegistry from '@coomi/coomi-agent'
import GoalService, { type Config as GoalDomainConfig } from '@coomi/coomi-goal'
import * as goalSession from '@coomi/coomi-goal-round-driver'
import * as toolGoal from '@coomi/coomi-tool-goal'
import LocalJobRegistry, { type Config as JobsConfig } from '@coomi/coomi-jobs-local'
import InvariantRegistry, { type Config as InvariantConfig } from '@coomi/coomi-invariants'
import * as sessionInvariant from '@coomi/coomi-session/invariant'
import * as agentInvariant from '@coomi/coomi-agent/invariant'
import * as scopeInvariant from '@coomi/coomi-scope/invariant'
import * as agentLoopInvariant from '@coomi/coomi-agent-loop/invariant'
import * as toolBash from '@coomi/coomi-tool-bash'
import * as bashEnv from '@coomi/coomi-shell-env'
import * as workspaceContext from '@coomi/coomi-agent-instructions'
import * as toolSkill from '@coomi/coomi-tool-skill'
import * as toolJobs from '@coomi/coomi-tool-jobs'
import AgentLoop, { type Config as AgentLoopConfig } from '@coomi/coomi-agent-loop'
import * as llmRetry from '@coomi/coomi-llm-retry'
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'coomiHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.coomiHome !== undefined ? { coomiHome: config.coomiHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  const nestedCoomiHome = config.skills?.filesystem?.coomiHome
  if (config.coomiHome !== undefined && nestedCoomiHome !== undefined
    && resolveCoomiHome(config.coomiHome) !== resolveCoomiHome(nestedCoomiHome)) {
    throw new Error('agent-spine-demo: coomiHome and skills.filesystem.coomiHome must resolve to the same directory')
  }
  const coomiHome = resolveCoomiHome(config.coomiHome ?? nestedCoomiHome)
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { coomiHome }))
  }
