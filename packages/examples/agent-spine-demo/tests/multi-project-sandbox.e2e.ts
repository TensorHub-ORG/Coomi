...
import { seatbeltProfileArgs } from '@coomi/coomi-sandbox-local/src/profiles.ts'
...
import type { ToolResult } from '@coomi/coomi-tools'
...
async function projectDir(label: string): Promise<string> {
  const dir = await mkdtemp(join(homedir(), `coomi-${label}-`))
  tempDirs.push(dir)
  return dir
}
