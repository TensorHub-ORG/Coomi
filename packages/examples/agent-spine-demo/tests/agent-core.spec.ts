...
import { renderPrompt, TOOL_ORDER_REST } from '@coomi/coomi-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@coomi/coomi-agent'
...
declare module '@coomi/coomi-jobs' {
...
/**
 * Unit coverage for the @coomi/coomi-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldCoomiHome = process.env.COOMI_HOME
  const oldAgentsHome = process.env.COOMI_AGENTS_HOME
  process.env.COOMI_HOME = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-home-'))
  process.env.COOMI_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldCoomiHome === undefined) {
      delete process.env.COOMI_HOME
    } else {
      process.env.COOMI_HOME = oldCoomiHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.COOMI_AGENTS_HOME
    } else {
      process.env.COOMI_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('coomi-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@coomi/coomi-agent$'] },
      { package_blocklist: ['^@coomi/coomi-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Coomi.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level coomiHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'coomi-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'coomi-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-coomi-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-coomi-home'),
      rootCallId: CallId('agent-core-coomi-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ COOMI_HOME: home, COOMI_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested coomi home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        coomiHome: '/global-coomi-home',
        workspaceContext: false,
        skills: { filesystem: { coomiHome: '/nested-coomi-home' } },
      })
    }).toThrow('agent-spine-demo: coomiHome and skills.filesystem.coomiHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'coomi-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
