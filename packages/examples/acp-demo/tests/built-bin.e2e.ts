...
async function makeConsumer(): Promise<string> {
  ...
  for (const rel of coomiPackages) {
...
describe.skipIf(!existsSync(acpBin))('coomi-acp-demo BUILT bin (node lib/bin.js, no tsx)', () => {
  it('boots the published bin, completes a turn, and writes default Zstandard persistence', async () => {
    ...
    child = spawn(process.execPath, [acpBin, '--config', './cordis.yml'], {
      ...
      env: {
        ...process.env,
        COOMI_HOME: join(consumer, '.coomi'),
        COOMI_AGENTS_HOME: join(consumer, '.agents'),
      },
