...
import { agentEvents } from '@coomi/coomi-agent'
import { TOOL_ORDER_REST } from '@coomi/coomi-system-prompt'
import type { Message } from '@coomi/coomi-llm'
import { SessionId } from '@coomi/coomi-session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @coomi/coomi-acp-demo composition:
...
async function mount(config: acpAgent.Config, withBash = false): Promise<Context> {
  ...
  config.persistenceRoot ??= await mkdtemp(join(tmpdir(), 'coomi-acp-demo-persistence-'))
...
async function isolatedSkillsConfig(catalogDescriptionMaxLength?: number): Promise<NonNullable<acpAgent.Config['skills']>> {
  const home = await mkdtemp(join(tmpdir(), 'coomi-acp-demo-skills-'))
  return {
    filesystem: { coomiHome: join(home, '.coomi'), agentsHome: join(home, '.agents') },
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  const home = await mkdtemp(join(tmpdir(), 'coomi-acp-demo-default-skills-'))
  process.env.COOMI_HOME = join(home, '.coomi')
  process.env.COOMI_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
...
describe('coomi-acp-demo composition', () => {
  it('brings up the spine + persistence + the ACP bridge', async () => {
    const ctx = await mount({
      provider: 'mock',
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/coomi-acp-demo-test',
  ...
  it('forwards explicit project-instruction controls to the bundled spine', async () => {
    const ctx = await mount({
      ...
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/coomi-acp-demo-workspace-context',
      workspaceContext: false,
    })
    expect(ctx.get('agents')).toBeDefined()
    expect(ctx.get('agentLoop')).toBeDefined()
  ...
  it('forwards skill config and coomiHome into agent-spine-demo', async () => {
    const skills = await isolatedSkillsConfig(6)
    const ctx = await mount({ provider: 'mock', model: 'mock', persona: 'hi', coomiHome: skills.filesystem!.coomiHome!, skills, workspaceContext: false })
  ...
  it('forwards maxParallelToolCalls to the bundled agent loop', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/coomi-acp-demo-test-parallel',
      skills: await isolatedSkillsConfig(),
      workspaceContext: false,
    })
  ...
  it('forwards toolOrder through agent-spine-demo to the system-prompt assembly', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/coomi-acp-demo-test-tool-order',
      workspaceContext: false,
    })
