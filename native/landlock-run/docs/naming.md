...
The public package family belongs to the `@coomi` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@coomi/node-addon-landlock-run
@coomi/node-addon-landlock-run-<platform>
```
