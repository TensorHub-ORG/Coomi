...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'COOMI_SNAPSHOT_MAX_CONCURRENCY',
...
if (process.env.COOMI_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.COOMI_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.COOMI_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
