...
class CoomiHarness:
    """Reusable synchronous SDK for running Coomi agent turns.
    ...
    def __init__(self, config: CoomiHarnessConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either CoomiHarnessConfig or keyword options, not both")
        self.config = config or CoomiHarnessConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["COOMI_SESSION_ROOT"] = self.config.session_root
        if self.config.cordis is not None:
            env["COOMI_CORDIS_CONFIG"] = self.config.cordis
        env["COOMI_CWD"] = cwd
    ...
    def __enter__(self) -> "CoomiHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: CoomiHarness, session_id: str) -> None:
