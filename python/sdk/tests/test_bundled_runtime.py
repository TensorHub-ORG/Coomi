...
from coomi_harness import CoomiHarness, HarnessClient, HarnessConfig
from coomi_harness.errors import TransportClosedError
...
- id: sdk-jsonrpc-server
  name: '@coomi/coomi-sdk-jsonrpc-server'
- id: agent-core
  name: '@coomi/coomi-agent-spine-demo'
  config:
    workspaceContext: false
- id: sessions
  name: '@coomi/coomi-session-persistence-jsonl'
  config:
    root: './sessions'
- id: session-checkpoints
  name: '@coomi/coomi-session-checkpoint-policy'
- id: subprocess
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> HarnessClient:
    return HarnessClient(
        HarnessConfig(
            ...
            cwd=str(tmp_path),
            env={
                "COOMI_CORDIS_CONFIG": "./cordis.yml",
                "COOMI_SESSION_ROOT": str(tmp_path / "sessions"),
                "COOMI_CWD": str(tmp_path),
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    assert init.serverInfo.name == "coomi-desktop-sdk-runtime"
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = CoomiHarness(
        ...
        env={
            "COOMI_MODEL": model,
            "COOMI_CONTEXT_WINDOW": "1000000",
            "COOMI_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@coomi/coomi-does-not-exist'\n"
    )
    ...
    assert "@coomi/coomi-does-not-exist" in str(excinfo.value)
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("COOMI_RUNTIME_MODE", mode)
