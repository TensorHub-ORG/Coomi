...
from coomi_harness import CoomiHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
json.dump({
    ...
    "COOMI_CWD": os.environ.get("COOMI_CWD"),
    "COOMI_SESSION_ROOT": os.environ.get("COOMI_SESSION_ROOT"),
    "COOMI_CORDIS_CONFIG": os.environ.get("COOMI_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["COOMI_CWD"] == str(tmp_path)
    assert dumped_env["COOMI_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["COOMI_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("COOMI_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with CoomiHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-coomi"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(
        ...
        assert init.serverInfo.name == "fake-coomi"
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from coomi_harness import CoomiHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(CoomiHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in CoomiHarnessConfig.__dataclass_fields__
    assert "max_tokens" in CoomiHarnessConfig.__dataclass_fields__
...
def _install_fake_bundled_runtime(
    ...
    """
    runtime = tmp_path / "coomi-jsonrpc-agent"
    runtime.write_text(
        """#!/usr/bin/env python3
import json
import os
...
json.dump({"COOMI_CORDIS_CONFIG": os.environ.get("COOMI_CORDIS_CONFIG")}, open(os.environ["ENV_DUMP"], "w"))
for line in sys.stdin:
    msg = json.loads(line)
    if msg.get("method") == "initialize":
...
""".strip()
    ...
    module_dir = tmp_path / "coomi_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "coomi_harness_runtime", raising=False)
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    if ambient_config is None:
        monkeypatch.delenv("COOMI_CORDIS_CONFIG", raising=False)
    else:
        monkeypatch.setenv("COOMI_CORDIS_CONFIG", ambient_config)
    ...
    assert json.loads(env_dump.read_text())["COOMI_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "COOMI_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="deepseek-official", cwd="/workspace", model="deepseek-v4-pro")

    assert json.loads(env_dump.read_text())["COOMI_CORDIS_CONFIG"] == "./explicit.yml"
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install coomi-desktop-runtime-bin"):
        HarnessClient().start()
