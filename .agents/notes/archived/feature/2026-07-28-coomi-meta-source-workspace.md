# Agent Note: `coomi meta` boots the TUI over the harness checkout

Status: implemented
Archived: 2026-08-03

English | [中文](2026-07-28-coomi-meta-source-workspace.zh.md)

## Problem

`coomi` treats the invoking directory as the workspace, which is what makes it useful on arbitrary projects. Working on coomi itself therefore means `cd`-ing to the checkout first — and the checkout is not a memorable path: the source install keeps it under a container directory as a timestamped staging worktree (`~/.coomi/source/staging-<timestamp>`) behind a `current` symlink, so the target moves on every upgrade. The agent is already *told* where its source lives by the `harness:source` prompt section, and the `cordis` toolset can modify that runtime, but the human still had to locate the directory by hand to start a session there.

## Decision

`coomi meta` boots the ordinary TUI with the harness checkout as the workspace, from any directory.
...
**Resolve `~/.coomi/source/current` instead of the launcher's own path.** Rejected: it would diverge from the `harness:source` prompt path whenever a non-installed checkout's `bin/coomi` is invoked directly, telling the model one source root while working in another.

## Consequences

Starting a session on coomi's own source is `coomi meta --experimental` from anywhere (or bare `coomi meta` under `COOMI_EXPERIMENTAL=1`), and the workspace is guaranteed to be the same checkout the model is told about. The command always starts fresh; an ordinary `coomi --resume <id>` later restores the session and enters its persisted workspace.
