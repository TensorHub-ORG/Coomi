...
Add `FileSystem.listDir(target, signal?)` to `@coomi/coomi-fs`.
...
**Keep directory enumeration in each consumer.** Rejected. That would bind product packages such as `coomi-skill` to Node/local filesystem behavior and bypass policy/remote/sandboxed backends.
