...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `coomi-tool-bash` (`bash`), `coomi-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `coomi-tool-subagent` (`subagent`). Example-only tools are excluded.
