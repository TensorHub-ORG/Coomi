...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `coomi-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `coomi-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `coomi-timeout` (`idleWatchdog`) |
| Clear the timer | `coomi-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `coomi-timeout` (`timeoutOf`) |
