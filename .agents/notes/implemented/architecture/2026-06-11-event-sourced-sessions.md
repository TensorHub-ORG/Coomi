...
- Persistence stays a plugin concern; the in-memory store ships in coomi-session.
...
- Derivation cost grows with log length — compaction (coomi-compaction) is the intended mitigation, not log mutation.
