# Agent Note: Omit runtime invariants from shipped coomi config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@coomi/coomi-*/invariant` entry.
...
- Ordinary `coomi` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
