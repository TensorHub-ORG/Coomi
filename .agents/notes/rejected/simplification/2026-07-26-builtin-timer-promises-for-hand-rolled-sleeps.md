...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`coomi-llm-mock-server` `pause()`, `coomi-lsp-stdio`, `coomi-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
