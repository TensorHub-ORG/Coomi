...
- **Six merge-extensible maps** (~370 LOC of core types): `ContentBlockMap`, `MessageSourceMap`, `FinishReasonMap` (in `coomi-llm`); `TurnTriggerMap`, `TurnEndReasonMap`, `SessionEventMap` (in `coomi-session`).
- **~10 `declare module` augmentation sites** across `coomi-agent`, `coomi-agent-loop`, `coomi-shell`, `coomi-llm`, `coomi-session`, `coomi-session-persistence`, `coomi-system-prompt`, `coomi-tools` — each would move from declaration merging to a runtime `register()` call.
- **The event producers** — 16 `session.append(...)` call sites in the loop — unchanged in shape but now validated at the boundary.
- **~7 switch-consumers** that branch on these unions: `deriveMessages` and the package-owned invariant companion (`coomi-session`), `BlockAssembler` (`coomi-llm`), both LLM adapters (`coomi-llm-deepseek`, `coomi-llm-pi-ai`), and the tool schema layer (`coomi-tools`). The `assertNever`-on-closed-unions vs fall-through-on-extensible-unions convention (a documented lint rule) would need rethinking — runtime variants are not statically exhaustive.
- **The `defineTool` `InferArgs` DSL** (`coomi-tools`), which derives zero-cast `execute` arg types from a compile-time schema spec — the showcase of the current approach.
